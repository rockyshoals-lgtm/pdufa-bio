...
def calculate_safe_batch_size(n_rows: int, free_vram_gb: float, safety_factor: float = 0.4) -> int:
    ...
    return max(10_000, min(max_batch, 2_000_000))
...
def sample_configs_adaptive(rng: np.random.Generator, sobol: SobolGenerator, n: int,
                            ...
                            explore_ratio: float = 0.3, sigma: float = 0.15,
                            ce_ratio: float = 0.0, ce_sigma: float = 0.3) -> np.ndarray:
    """
    Adaptive sampling with four modes:
    1. Sobol quasi-random exploration (explore_ratio * 0.5)
    2. Pure random exploration (explore_ratio * 0.5)
    3. Cross-entropy sampling around best configs (ce_ratio)
    4. Local refinement around best configs ((1 - explore_ratio) - ce_ratio)

    Parameters:
        rng: NumPy random generator for reproducibility
        sobol: SobolGenerator instance for quasi-random points
        n: number of samples to generate
        param_keys: list of parameter names corresponding to bounds
        hall: hall-of-fame list of previous best entries (used for local and CE sampling)
        explore_ratio: fraction of points used for global exploration (Sobol + random)
        sigma: initial sigma for Gaussian perturbations in local refinement
        ce_ratio: fraction of points drawn from cross-entropy distribution around hall-of-fame
        ce_sigma: scale factor for CE sampling (controls spread around mean of hall)

    Returns:
        np.ndarray of shape (n, len(param_keys)) with sampled parameter values.
    """
    P = len(param_keys)
    ...
    # Determine counts for each sampling strategy
    n_sobol = int(n * explore_ratio * 0.5)
    n_random = int(n * explore_ratio * 0.5)
    # Cross-entropy portion (drawn from distribution of hall-of-fame)
    n_ce = int(n * ce_ratio)
    # Local refinement gets the remainder
    n_local = n - n_sobol - n_random - n_ce
    ...
    if n_sobol > 0:
        ...
        out[idx_start:idx_start+n_sobol] = lo + sobol_points[:, :P] * span
        idx_start += n_sobol

    # 2. Pure random for diversity
    if n_random > 0:
        out[idx_start:idx_start+n_random] = lo + rng.random((n_random, P), dtype=np.float32) * span
        idx_start += n_random

    # 3. Cross-entropy sampling around hall-of-fame
    if n_ce > 0 and hall:
        ...
        mu = seed_mat.mean(axis=0)
        sigma_vec = seed_mat.std(axis=0) * ce_sigma
        sigma_vec = np.where(sigma_vec < 1e-3 * span, 1e-3 * span, sigma_vec)
        # Sample from independent Gaussians
        ce_samples = rng.normal(loc=mu, scale=sigma_vec, size=(n_ce, P)).astype(np.float32)
        ce_samples = np.clip(ce_samples, lo, hi)
        out[idx_start:idx_start+n_ce] = ce_samples
        idx_start += n_ce
    elif n_ce > 0:
        # If no hall yet, default to random
        out[idx_start:idx_start+n_ce] = lo + rng.random((n_ce, P), dtype=np.float32) * span
        idx_start += n_ce

    # 4. Local refinement around best configs
    if n_local > 0 and hall:
        ...
        # Noise: use standard normal by default; heavy-tailed distribution to explore further
        noise = rng.standard_normal((n_local, P)).astype(np.float32) * sigma * span
        out[idx_start:idx_start+n_local] = np.clip(base + noise, lo, hi)
    elif n_local > 0:
        out[idx_start:idx_start+n_local] = lo + rng.random((n_local, P), dtype=np.float32) * span
...
def main():
    ...
    # New argument: safety factor for GPU memory usage
    ap.add_argument("--safety-factor", type=float, default=0.6,
                    help="Fraction of free GPU memory to use when auto-calculating batch size (default 0.60)")
    # Cross-entropy sampling controls
    ap.add_argument("--ce-ratio", type=float, default=0.0,
                    help="Fraction of samples drawn from cross-entropy distribution around the hall-of-fame (default 0.0)")
    ap.add_argument("--ce-sigma", type=float, default=0.3,
                    help="Scale factor for cross-entropy sampling; larger values sample wider around the hall (default 0.3)")
    # Precision for GPU computations (float32 or float16)
    ap.add_argument("--precision", choices=["float32", "float16"], default="float32",
                    help="Precision for GPU computations: float32 (default) or float16")
    args = ap.parse_args()
    ...
    if use_gpu:
        ...
        dtype = cp.float16 if args.precision == "float16" else cp.float32
        # Transfer features to GPU with selected precision
        F = xp.asarray(F_np, dtype=dtype)
        # y_xp kept in float32 for metrics regardless of F precision
    ...
    # Determine dtype for weights on GPU based on precision
    if use_gpu:
        weight_dtype = cp.float16 if args.precision == "float16" else cp.float32
    else:
    ...
    if args.batch > 0:
        ...
        batch_size = calculate_safe_batch_size(n_rows, free_vram, safety_factor=args.safety_factor)
    else:
        batch_size = 100_000
    ...
    for attempt in range(5):
        try:
            ...
            W = test[:, w_idx].T
            if use_gpu:
                # Cast warm-up weights to desired dtype based on precision
                W = xp.asarray(W, dtype=weight_dtype)
            pmat = F.dot(W)
            # Cast pmat to float32 when using float16 precision
            if use_gpu and args.precision == "float16":
                pmat = pmat.astype(cp.float32)
    ...
    while processed < total:
        ...
        try:
            ...
            params_mat = sample_configs_adaptive(
                ...
                ce_ratio=args.ce_ratio, ce_sigma=args.ce_sigma
            )
            ...
            if use_gpu:
                # Cast weights and thresholds to selected precision
                W = xp.asarray(W, dtype=weight_dtype)
                p_base = xp.asarray(p_base, dtype=weight_dtype)
                p_thr = xp.asarray(p_thr, dtype=weight_dtype)
            
            # Core computation
            pmat = p_base[None, :] + F.dot(W)
            # When using float16 precision, cast pmat to float32 for stable metric computations
            if use_gpu and args.precision == "float16":
                pmat = pmat.astype(cp.float32)
            ...
            # Get top configs
            # Compute feasible count and top indices differently for GPU vs CPU
            if use_gpu:
                # Count feasible configs on GPU
                n_feasible_batch = int(((fit > -1e17).sum()).get())
                total_feasible += n_feasible_batch

                topM = min(256, cur)
                if topM > 0:
                    # Get top indices on GPU using argpartition and argsort
                    top_idx_gpu = cp.argpartition(-fit, max(0, topM - 1))[:topM]
                    top_idx_gpu = top_idx_gpu[cp.argsort(-fit[top_idx_gpu])]
                    top_idx_cpu = top_idx_gpu.get()
                else:
                    top_idx_cpu = np.array([], dtype=int)

                improved = False
                if len(top_idx_cpu) > 0:
                    # Pre-fetch metrics for top indices in bulk
                    tp_top = cp.asnumpy(tp[top_idx_gpu])
                    fp_top = cp.asnumpy(fp[top_idx_gpu])
                    precision_top = cp.asnumpy(precision[top_idx_gpu])
                    recall_top = cp.asnumpy(recall[top_idx_gpu])
                    f1_top = cp.asnumpy(f1[top_idx_gpu])
                    specificity_top = cp.asnumpy(specificity[top_idx_gpu])
                    mcc_top = cp.asnumpy(mcc[top_idx_gpu])
                    brier_top = cp.asnumpy(brier[top_idx_gpu])

                    for j, idx in enumerate(top_idx_cpu):
                        # Skip infeasible configs (fitness below threshold)
                        if fit[idx].get() < -1e17:
                            continue
                        ...
                        metrics = {
                            "precision": float(precision_top[j]),
                            "recall": float(recall_top[j]),
                            "f1": float(f1_top[j]),
                            "specificity": float(specificity_top[j]),
                            "mcc": float(mcc_top[j]),
                            "brier": float(brier_top[j]),
                            "tp": int(tp_top[j]),
                            "fp": fp_val,
                        }
                        entry = pack_entry(params, metrics, float(fit[idx].get()))
