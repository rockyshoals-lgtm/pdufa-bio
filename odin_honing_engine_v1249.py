...
"""
╔══════════════════════════════════════════════════════════════════════════╗
║                    ODIN HONING ENGINE v5.2                             ║
║   Fast CRS (cached logits) + Annealed SWP + Better Selection           ║
║                                                                         ║
║  Key upgrades vs v5.1:                                                  ║
║   1. FAST CRS: O(N) incremental trial eval (no X@w per trial)           ║
║   2. FAST SWP: cached logits + simulated annealing acceptance           ║
║   3. WF regularizer defaults to recent folds (more future-robust)       ║
║   4. Freeze dead columns (all-zero features) to save budget             ║
║   5. Dynamic historical_crl_rate if missing/flat (T-1 safe)             ║
║   6. Correct ensemble metadata + best-of-both output (single vs ens)    ║
║   7. Calibration fit on TRAIN (not VAL)                                 ║
║                                                                         ║
║  Optional CLI:                                                          ║
║    --seed <int>          reproducible runs                              ║
║    --out_prefix v####    write odin_v####_{honed,ensemble,best,anchor}.json║
║    --update_anchor       overwrite DEFAULT_ANCHOR with selected best    ║
╚══════════════════════════════════════════════════════════════════════════╝
"""
...
OUTPUT_HONED    = "odin_v1249_honed.json"
OUTPUT_ENSEMBLE = "odin_v1249_honed_ensemble.json"
OUTPUT_BEST     = "odin_v1249_best.json"
...
def _compute_dynamic_hist_crl_rate(df: pd.DataFrame,
                                  alpha: float = 2.0,
                                  beta: float = 2.0,
                                  ta_col: str = 'therapeutic_area',
                                  date_col: str = 'catalyst_date',
                                  label_col: str = 'label') -> pd.DataFrame:
    """
    Compute T-1 safe historical CRL rate per therapeutic area using ONLY prior-dated events.
    Events on the same catalyst_date do NOT "see" each other (no same-day leakage).
    Uses a Beta(alpha, beta) prior for smoothing.
    """
    if ta_col not in df.columns or label_col not in df.columns:
        return df

    # Robust event date: prefer catalyst_date, else fall back to mid-year
    dt = pd.to_datetime(df.get(date_col, pd.NaT), errors='coerce')
    if dt.isna().all():
        dt = pd.to_datetime(df['year'].astype(str) + '-07-01', errors='coerce')
    dt = dt.fillna(pd.to_datetime(df['year'].astype(str) + '-07-01', errors='coerce'))
    dt = dt.fillna(pd.Timestamp('2020-07-01'))

    work = df.copy()
    work['_tmp_date'] = dt
    work = work.sort_values('_tmp_date')

    from collections import defaultdict
    tot = defaultdict(int)
    crl = defaultdict(int)

    hist = pd.Series(index=work.index, dtype=float)

    # Group by date to avoid same-day leakage
    for _, g in work.groupby('_tmp_date', sort=True):
        # 1) assign rates based on strictly prior history
        for idx in g.index:
            ta = str(work.at[idx, ta_col]).strip()
            t = tot[ta]
            c = crl[ta]
            denom = t + alpha + beta
            hist.at[idx] = (c + alpha) / denom if denom > 0 else 0.30

        # 2) update counts with the outcomes of this date
        for idx in g.index:
            ta = str(work.at[idx, ta_col]).strip()
            tot[ta] += 1
            if float(work.at[idx, label_col]) < 0.5:
                crl[ta] += 1

    # Reindex back to original order
    out = df.copy()
    out['historical_crl_rate'] = hist.reindex(df.index).astype(float)
    return out


def load_data(path: str,
              compute_dynamic_hist: bool = True,
              hist_prior_alpha: float = 2.0,
              hist_prior_beta: float = 2.0) -> tuple:
    """Load CSV and return (X, y, dates) arrays."""
    df = pd.read_csv(path, low_memory=False)
    print(f"[Data] Loaded {len(df)} rows from {path}")

    # Label
    df['label'] = (df['outcome'].astype(str).str.upper() == 'APPROVAL').astype(float)

    # Catalyst year for time splits
    df['year'] = pd.to_datetime(df['catalyst_date'], errors='coerce').dt.year.fillna(2020).astype(int)

    # Optional: compute dynamic historical CRL rate (T-1 safe) if missing/flat
    if compute_dynamic_hist:
        needs_hist = ('historical_crl_rate' not in df.columns)
        if not needs_hist:
            # If the column is effectively constant, replace it with a dynamic version
            nunique = df['historical_crl_rate'].nunique(dropna=False)
            needs_hist = nunique <= 3
        if needs_hist:
            df = _compute_dynamic_hist_crl_rate(df,
                                                alpha=hist_prior_alpha,
                                                beta=hist_prior_beta)
            print("[Data] Computed dynamic historical_crl_rate (T-1 safe, per-TA).")
...
def brier_from_logits(y_true: np.ndarray, logits: np.ndarray) -> float:
    """Brier score computed directly from logits (avoids X@w recompute)."""
    p = sigmoid(logits)
    return float(np.mean((p - y_true) ** 2))


def build_wf_test_indices(dates: np.ndarray,
                          n_folds: int = 5,
                          max_folds: int = 3,
                          mode: str = "last") -> list:
    """
    Build walk-forward test indices for a quick out-of-sample regularization term.

    mode:
      - "last": use the most recent folds (recommended for 'future robustness')
      - "first": use the earliest folds (legacy)
      - "uniform": spread folds across the timeline
    """
    ...
    if mode == "last":
        fold_ids = list(range(max(0, n_folds - max_folds), n_folds))
    elif mode == "uniform":
        k = min(max_folds, n_folds)
        fold_ids = np.linspace(0, n_folds - 1, num=k, dtype=int).tolist()
    else:  # "first" or anything else
        fold_ids = list(range(min(max_folds, n_folds)))

    te_indices = []
    for fold in fold_ids:
        cutoff_idx = fold_size * (fold + 1)
        test_years = years[cutoff_idx:cutoff_idx + fold_size]
        if len(test_years) == 0:
            continue
        te_mask = np.isin(dates, test_years)
        if te_mask.sum() >= 5:
            te_indices.append(np.where(te_mask)[0])
    return te_indices
...
def run_crs(X_tr, y_tr, X_val, y_val, w_init, anchor_w, cfg, sign_mask,
            floor_mask=None, X_all=None, y_all=None, dates_all=None,
            cap_mask=None, trainable_idx=None):
    """
    Coordinate Random Search (CRS) — FAST VERSION.

    Key upgrade vs earlier CRS:
      - Uses cached logits (z = X@w) and O(N) incremental updates for single-weight trials.
      - Inner-loop objective defaults to Brier-based terms only (AUC is too noisy/slow).
      - Optional walk-forward "regularizer" uses recent folds by default.

    This makes it cheap enough to increase trial budgets over many perpetual runs without
    paying the matrix-multiply cost per trial.
    """
    w = w_init.copy()

    # Objective weights (minimize score)
    val_w = cfg.get('crs_val_brier_weight', 1.0)
    tr_w  = cfg.get('crs_train_brier_weight', 0.20)
    wf_w  = cfg.get('crs_wf_brier_weight', 0.30)

    sweeps    = cfg.get('crs_sweeps', 30)
    trials    = cfg.get('crs_trials_per_weight', 60)
    ...
    # Anchor regularization (can be per-feature scaled for rare columns)
    base_lam  = cfg.get('crs_anchor_lambda', 0.001)
    anc_vec   = np.full(len(w), base_lam, dtype=np.float64)

    rare_thr  = cfg.get('rare_feature_threshold', 0.01)  # 1% non-zero prevalence
    rare_mult = cfg.get('anchor_lambda_rare_mult', 0.0)  # 0 disables
    if rare_mult and rare_mult > 0:
        prev = np.mean(np.abs(X_tr) > 1e-12, axis=0)
        anc_vec[prev < rare_thr] *= float(rare_mult)

    # Choose trainable indices (skip all-zero columns)
    if trainable_idx is None:
        trainable_idx = np.arange(len(w), dtype=int)

    # Cached logits
    z_val = X_val @ w
    z_tr  = (X_tr @ w) if tr_w > 0 else None

    use_wf = (wf_w > 0) and (X_all is not None) and (y_all is not None) and (dates_all is not None)
    if use_wf:
        z_all = X_all @ w
        wf_folds = cfg.get('wf_folds', 5)
        wf_mode  = cfg.get('crs_wf_mode', 'last')           # NEW: default to recent folds
        wf_max   = cfg.get('crs_wf_max_folds', 3)
        wf_te_idx = build_wf_test_indices(dates_all, n_folds=wf_folds,
                                          max_folds=wf_max, mode=wf_mode)
        # Pre-slice y for speed
        wf_y = [y_all[idx] for idx in wf_te_idx]
    else:
        z_all = None
        wf_te_idx = []
        wf_y = []

    # Anchor penalty current
    anchor_pen = float(np.sum(anc_vec * (w - anchor_w) ** 2))

    def wf_brier_for_delta(idx: int, delta: float) -> float:
        if not wf_te_idx:
            return 0.0
        b_list = []
        xcol_all = X_all[:, idx]
        for te_idx, y_te in zip(wf_te_idx, wf_y):
            z_te = z_all[te_idx] + delta * xcol_all[te_idx]
            b_list.append(brier_from_logits(y_te, z_te))
        return float(np.mean(b_list)) if b_list else 0.0

    # Current score
    cur_val_b = brier_from_logits(y_val, z_val)
    cur_tr_b  = brier_from_logits(y_tr, z_tr) if (tr_w > 0 and z_tr is not None) else 0.0
    cur_wf_b  = wf_brier_for_delta(0, 0.0) if wf_te_idx else 0.0
    best_score = val_w * cur_val_b + tr_w * cur_tr_b + wf_w * cur_wf_b + anchor_pen

    total_improved = 0

    for sweep in range(sweeps):
        order = np.random.permutation(trainable_idx)
        for idx in order:
            ...
            for _ in range(trials):
                candidate = w_old + np.random.randn() * sigma
                ...
                # Magnitude clamp + caps
                candidate = float(np.clip(candidate, -mag_clamp, mag_clamp))
                if cap_mask is not None:
                    cap = cap_mask[idx]
                    if cap > 0:
                        candidate = float(np.clip(candidate, -cap, cap))

                # Near-zero guard
                if s_constraint > 0 and candidate < 1e-6:
                    candidate = 1e-6
                elif s_constraint < 0 and candidate > -1e-6:
                    candidate = -1e-6

                delta = candidate - w_old
                if abs(delta) < 1e-12:
                    continue

                # Logits update (trial, not committed)
                z_val_t = z_val + delta * X_val[:, idx]
                val_b   = brier_from_logits(y_val, z_val_t)

                tr_b = 0.0
                if tr_w > 0 and z_tr is not None:
                    z_tr_t = z_tr + delta * X_tr[:, idx]
                    tr_b   = brier_from_logits(y_tr, z_tr_t)

                wf_b = wf_brier_for_delta(idx, delta) if wf_w > 0 else 0.0

                # Anchor penalty update (O(1))
                ap_t = anchor_pen + anc_vec[idx] * (
                    (candidate - anchor_w[idx]) ** 2 - (w_old - anchor_w[idx]) ** 2
                )

                score_t = val_w * val_b + tr_w * tr_b + wf_w * wf_b + ap_t

                if score_t < best_local_score:
                    best_local_score = score_t
                    best_wi = candidate
                    best_delta = delta

            # Commit best move for this coordinate
            if abs(best_delta) > 0:
                w[idx] = best_wi
                z_val = z_val + best_delta * X_val[:, idx]
                if tr_w > 0 and z_tr is not None:
                    z_tr = z_tr + best_delta * X_tr[:, idx]
                if wf_te_idx:
                    z_all = z_all + best_delta * X_all[:, idx]

                anchor_pen = anchor_pen + anc_vec[idx] * (
                    (best_wi - anchor_w[idx]) ** 2 - (w_old - anchor_w[idx]) ** 2
                )
...
def run_swp(X_tr, y_tr, X_val, y_val, w_init, cfg, sign_mask,
            floor_mask=None, cap_mask=None,
            X_all=None, y_all=None, dates_all=None,
            trainable_idx=None, anchor_w=None):
    """
    SWP — Stochastic Weight Perturbation (FAST + annealed accept).

    v5.2 upgrade:
      - cached logits + single-coordinate O(N) updates (no X@w per step)
      - optional WF regularizer on recent folds (same helper as CRS)
      - simulated annealing acceptance to escape small local minima
    """
    w = w_init.copy()

    val_w = cfg.get('swp_val_brier_weight', 1.0)
    tr_w  = cfg.get('swp_train_brier_weight', 0.0)   # default: val-only
    wf_w  = cfg.get('swp_wf_brier_weight', 0.10)
    base_lam = cfg.get('swp_anchor_lambda', 0.0)     # default: off (CRS already anchors)
    ...
    if use_wf:
        z_all = X_all @ w
        wf_folds = cfg.get('wf_folds', 5)
        wf_mode  = cfg.get('swp_wf_mode', 'last')
        wf_max   = cfg.get('swp_wf_max_folds', 2)
        wf_te_idx = build_wf_test_indices(dates_all, n_folds=wf_folds,
                                          max_folds=wf_max, mode=wf_mode)
        wf_y = [y_all[idx] for idx in wf_te_idx]
    else:
    ...
    # Optional anchor penalty (rarely needed)
    if base_lam and anchor_w is not None:
        anchor_pen = float(base_lam * np.sum((w - anchor_w) ** 2))
    else:
        anchor_pen = 0.0
    ...
    # Objective for current weights
    cur_val_b = brier_from_logits(y_val, z_val)
    cur_tr_b  = brier_from_logits(y_tr, z_tr) if (tr_w > 0 and z_tr is not None) else 0.0
    cur_wf_b  = wf_brier_for_delta(0, 0.0) if wf_te_idx else 0.0
    cur_score = val_w * cur_val_b + tr_w * cur_tr_b + wf_w * cur_wf_b + anchor_pen
    ...
    temp       = cfg.get('swp_temp', 0.0008)
    temp_decay = cfg.get('swp_temp_decay', 0.9995)
    temp_floor = 1e-7

    for _ in range(iterations):
        idx = int(np.random.choice(trainable_idx))
        s_constraint = sign_mask[idx]
        floor = floor_mask[idx] if floor_mask is not None else 0.0
        ...
        if abs(delta) < 1e-12:
            sigma *= decay
            temp = max(temp_floor, temp * temp_decay)
            continue
        ...
        ap_t = anchor_pen
        if base_lam and anchor_w is not None:
            ap_t = float(base_lam * np.sum((w - anchor_w) ** 2))  # small dim, ok

        score_t = val_w * val_b + tr_w * tr_b + wf_w * wf_b + ap_t

        # Annealed accept/reject
        accept = (score_t <= cur_score)
        if not accept:
            # accept worse moves with probability exp(-Δ/T)
            delta_s = score_t - cur_score
            if temp > temp_floor:
                p_accept = np.exp(-delta_s / temp)
...
def train(args):
    ...
    if os.path.exists(cfg_path):
        ...
        # Optional: allow runner to override selection metric without editing config
    env_sel = os.environ.get("ODIN_SELECTION_METRIC")
    if env_sel:
        cfg['selection_metric'] = str(env_sel).strip()
        print(f"[Config] selection_metric overridden by env → {cfg['selection_metric']}")
...
# ── Load anchor ──────────────────────────────────────────────────────────
    ...
    # ── RNG seed (reproducible exploration) ───────────────────────────────────
    seed = args.seed if getattr(args, "seed", None) is not None else int(time.time() * 1e6) % (2**32 - 1)
    np.random.seed(seed)
    print(f"[Seed] {seed}")
    ...
    X, y, dates = load_data(
        data_path,
        compute_dynamic_hist=cfg.get('compute_dynamic_hist_crl_rate', True),
        hist_prior_alpha=cfg.get('hist_prior_alpha', 2.0),
        hist_prior_beta=cfg.get('hist_prior_beta', 2.0),
    )
    ...
    cap_mask = build_cap_mask()
        # ── Trainable mask (skip all-zero columns to avoid wasting CRS/SWP budget) ─
    if cfg.get('freeze_dead_columns', True):
        # A column is "dead" if it's identically zero in BOTH train and val
        nz = (np.any(np.abs(X_tr) > 1e-12, axis=0) | np.any(np.abs(X_val) > 1e-12, axis=0))
        trainable_mask = nz.astype(np.float32)
        trainable_idx = np.where(nz)[0].astype(int)
        dead = [FEATURE_NAMES[i] for i in range(len(FEATURE_NAMES)) if not nz[i]]
        if dead:
            print(f"[Mask] Freezing {len(dead)} dead weights (all-zero features in this dataset):")
            print("       " + ", ".join(dead[:12]) + (" ..." if len(dead) > 12 else ""))
    else:
        trainable_mask = np.ones(len(FEATURE_NAMES), dtype=np.float32)
        trainable_idx = np.arange(len(FEATURE_NAMES), dtype=int)
    ...
    if cfg.get('crs_enabled', True):
        ...
        for crs_run in range(n_runs):
            ...
            w_c, n_imp = run_crs(X_tr, y_tr, X_val, y_val, seed_w, anchor_w, cfg, sign_mask,
                                 ...
                                 X_all=X, y_all=y, dates_all=dates, trainable_idx=trainable_idx)
    ...
    if cfg.get('swp_enabled', True):
        ...
        w_swp = run_swp(X_tr, y_tr, X_val, y_val, best_w, cfg, sign_mask,
                      floor_mask=floor_mask, cap_mask=cap_mask,
                      X_all=X, y_all=y, dates_all=dates,
                      trainable_idx=trainable_idx, anchor_w=anchor_w)
    ...
    p_val_anchor = predict(X_val, anchor_w)
    baseline_val_brier = brier_score(y_val, p_val_anchor)
    baseline_val_auc   = roc_auc(y_val, p_val_anchor)
    baseline_wf_auc, baseline_wf_brier = walk_forward_eval(X, y, dates, anchor_w,
                                                          n_folds=cfg.get('wf_folds', 5))
    # Also store into anchor for downstream metadata fields
    anchor['_baseline_val_brier'] = float(baseline_val_brier)
    anchor['_baseline_val_auc']   = float(baseline_val_auc)
    anchor['_baseline_wf_auc']    = float(baseline_wf_auc)
    anchor['_baseline_wf_brier']  = float(baseline_wf_brier)

    print("\n[Baseline] Anchor metrics:")
    print(f"  Val  AUC={baseline_val_auc:.4f}  Brier={baseline_val_brier:.5f}")
    print(f"  WF   AUC={baseline_wf_auc:.4f}  Brier={baseline_wf_brier:.5f}")

    # ── Evaluate best single (post CRS/SWP/BasinHop) ─────────────────────────
    print("\n[Walk-Forward] Evaluating best single...")
    p_val_best = predict(X_val, best_w)
    val_auc_best   = roc_auc(y_val, p_val_best)
    val_brier_best = brier_score(y_val, p_val_best)
    wf_auc_best, wf_brier_best = walk_forward_eval(X, y, dates, best_w,
                                                  n_folds=cfg.get('wf_folds', 5))
    print(f"[Best Single] Val AUC={val_auc_best:.4f}  Brier={val_brier_best:.5f} | "
          f"WF AUC={wf_auc_best:.4f}  Brier={wf_brier_best:.5f}")

    # ── Ensemble weights (post candidates only) ──────────────────────────────
    ens_w = post_ensemble.ensemble_weights()
    if ens_w is None:
        ens_w = best_w.copy()

    p_val_ens = predict(X_val, ens_w)
    val_auc_ens   = roc_auc(y_val, p_val_ens)
    val_brier_ens = brier_score(y_val, p_val_ens)
    wf_auc_ens, wf_brier_ens = walk_forward_eval(X, y, dates, ens_w,
                                                n_folds=cfg.get('wf_folds', 5))
    print(f"[Ensemble]   Val AUC={val_auc_ens:.4f}  Brier={val_brier_ens:.5f} | "
          f"WF AUC={wf_auc_ens:.4f}  Brier={wf_brier_ens:.5f} "
          f"({len(post_ensemble.snapshots)} candidates)")

    # If ensemble is materially worse on validation, fall back immediately
    if val_brier_ens > val_brier_best * 1.01:
        print("[Ensemble] Ensemble >1% worse on Val Brier — forcing best single for outputs.")
        ens_w = best_w.copy()
        val_auc_ens, val_brier_ens, wf_auc_ens, wf_brier_ens = val_auc_best, val_brier_best, wf_auc_best, wf_brier_best

    # ── Choose output model (single vs ensemble) ──────────────────────────────
    selection_metric = cfg.get('selection_metric', 'wf_brier')  # wf_brier tends to be more stable across perpetual runs

    def _selection_score(metric: str, val_auc: float, val_brier: float, wf_auc: float, wf_brier: float) -> float:
        metric = (metric or '').lower()
        if metric == 'val_brier':
            return float(val_brier)
        if metric == 'wf_brier':
            return float(wf_brier)
        if metric == 'val_auc':
            return float(-val_auc)  # higher AUC is better
        if metric == 'wf_auc':
            return float(-wf_auc)
        # default: blended brier (slightly prefer WF)
        return float(0.65 * wf_brier + 0.35 * val_brier)

    score_single = _selection_score(selection_metric, val_auc_best, val_brier_best, wf_auc_best, wf_brier_best)
    score_ens    = _selection_score(selection_metric, val_auc_ens,  val_brier_ens,  wf_auc_ens,  wf_brier_ens)

    if score_ens < score_single:
        final_w = ens_w.copy()
        final_kind = 'ensemble'
        final_val_auc, final_val_brier, final_wf_auc, final_wf_brier = val_auc_ens, val_brier_ens, wf_auc_ens, wf_brier_ens
    else:
        final_w = best_w.copy()
        final_kind = 'best_single'
        final_val_auc, final_val_brier, final_wf_auc, final_wf_brier = val_auc_best, val_brier_best, wf_auc_best, wf_brier_best

    print(f"[Select] Using {final_kind} for outputs (metric={selection_metric}, score={min(score_single, score_ens):.6f})")

    # ── Calibration (fit on TRAIN; apply to Val + WF) ────────────────────────
    iso_cal = None
    cal_val_brier = final_val_brier
    cal_wf_brier  = final_wf_brier
    if cfg.get('calibration_enabled', True):
        cal_method = cfg.get('calibration_method', 'platt')
        print(f"\n[Calibration] Fitting {cal_method} on TRAIN predictions...")
        p_tr = predict(X_tr, final_w)
        cal_candidate, _, status = fit_calibration(y_tr, p_tr, method=cal_method)
        if cal_candidate is None and cfg.get('calibration_fallback_isotonic', True):
            print(f"[Calibration] {cal_method}: {status} — trying isotonic fallback")
            cal_method = 'isotonic'
            cal_candidate, _, status = fit_calibration(y_tr, p_tr, method='isotonic')

        if cal_candidate is None:
            print(f"[Calibration] Skipping calibration: {status}")
        else:
            # Apply to validation
            p_val = predict(X_val, final_w)
            if cal_method == 'platt':
                p_val_cal = apply_platt(p_val, cal_candidate)
            else:
                p_val_cal = apply_isotonic(p_val, cal_candidate)

            cal_val_brier = brier_score(y_val, p_val_cal)

            # Apply to walk-forward fold tests (same folds as walk_forward_eval)
            years = np.sort(np.unique(dates))
            ...
            for fold in range(cfg.get('wf_folds', 5)):
                ...
                if not test_years:
                    continue
                te_mask = np.isin(dates, list(test_years))
                if te_mask.sum() < 5:
                    continue
                p_te = predict(X[te_mask], final_w)
                if cal_method == 'platt':
                    p_te = apply_platt(p_te, cal_candidate)
                else:
                    p_te = apply_isotonic(p_te, cal_candidate)
                cal_briers.append(brier_score(y[te_mask], p_te))
            if cal_briers:
                cal_wf_brier = float(np.mean(cal_briers))

            iso_cal = cal_candidate
            gain = final_val_brier - cal_val_brier
            print(f"[Calibration] Val Brier: {final_val_brier:.5f} → {cal_val_brier:.5f} (Δ {gain:+.5f})")
            print(f"[Calibration] WF  Brier: {final_wf_brier:.5f} → {cal_wf_brier:.5f}")

    # ── Outputs + metadata ────────────────────────────────────────────────────
    ts_now = datetime.now().isoformat()

    # Resolve output paths (support perpetual runner)
    out_prefix = getattr(args, 'out_prefix', None)
    if out_prefix:
        out_honed_path    = f"odin_{out_prefix}_honed.json"
        out_ensemble_path = f"odin_{out_prefix}_honed_ensemble.json"
        out_best_path     = f"odin_{out_prefix}_best.json"
        out_anchor_path   = f"odin_{out_prefix}_anchor.json"
    else:
        out_honed_path    = getattr(args, 'out_honed', None) or OUTPUT_HONED
        out_ensemble_path = getattr(args, 'out_ensemble', None) or OUTPUT_ENSEMBLE
        out_best_path     = getattr(args, 'out_best', None) or OUTPUT_BEST
        out_anchor_path   = getattr(args, 'out_anchor', None)
    ...
    md_single = {
        ...
        "data_file": data_path,
        "model_type": "pdufa",
        "baseline_val_brier": float(baseline_val_brier),
        "baseline_val_auc": float(baseline_val_auc),
        "baseline_wf_brier": float(baseline_wf_brier),
        "baseline_wf_auc": float(baseline_wf_auc),
        "final_val_brier": float(val_brier_best),
        "final_val_auc": float(val_auc_best),
        "walk_forward_mean_brier": float(wf_brier_best),
        "walk_forward_mean_auc": float(wf_auc_best),
        "brier_improvement": float(baseline_val_brier - val_brier_best),
        "rounds_completed": 1,
        "selection_metric": selection_metric,
        "trainable_weights": int(len(trainable_idx)),
    }

    honed_out = weights_to_json(best_w, anchor, iso_cal or {}, md_single)
    with open(out_honed_path, 'w') as f:
        json.dump(honed_out, f, indent=2, cls=NumpyEncoder)
    print(f"\n[Output] Saved honed weights → {out_honed_path}")

    # Ensemble metadata
    md_ens = dict(md_single)
    md_ens.update({
        "type": "ensemble_top_k",
        "final_val_brier": float(val_brier_ens),
        "final_val_auc": float(val_auc_ens),
        "walk_forward_mean_brier": float(wf_brier_ens),
        "walk_forward_mean_auc": float(wf_auc_ens),
        "brier_improvement": float(baseline_val_brier - val_brier_ens),
    })

    ens_out = weights_to_json(ens_w, anchor, iso_cal or {}, md_ens)
    with open(out_ensemble_path, 'w') as f:
        json.dump(ens_out, f, indent=2, cls=NumpyEncoder)
    print(f"[Output] Saved ensemble weights → {out_ensemble_path}")

    # Best-of-both output (what the perpetual runner should anchor to)
    md_best = dict(md_single)
    md_best.update({
        "type": f"selected_{final_kind}",
        "final_val_brier": float(final_val_brier),
        "final_val_auc": float(final_val_auc),
        "walk_forward_mean_brier": float(final_wf_brier),
        "walk_forward_mean_auc": float(final_wf_auc),
        "calibrated_val_brier": float(cal_val_brier),
        "calibrated_wf_brier_mean": float(cal_wf_brier),
    })

    best_out = weights_to_json(final_w, anchor, iso_cal or {}, md_best)
    with open(out_best_path, 'w') as f:
        json.dump(best_out, f, indent=2, cls=NumpyEncoder)
    print(f"[Output] Saved selected best → {out_best_path}")

    # Optional: write next anchor
    if getattr(args, 'update_anchor', False) or (out_anchor_path is not None):
        if out_anchor_path is None:
            out_anchor_path = DEFAULT_ANCHOR
        with open(out_anchor_path, 'w') as f:
            json.dump(best_out, f, indent=2, cls=NumpyEncoder)
        print(f"[Output] Updated anchor → {out_anchor_path}")
    # Keep downstream state/save/summary consistent with the selected output
    best_w = final_w.copy()
    best_val_brier = float(final_val_brier)
    wf_auc = float(final_wf_auc)
    wf_brier = float(final_wf_brier)
    cal_brier = float(cal_val_brier)
    cal_wf_brier = float(cal_wf_brier)
# Final save of state
    save_state(STATE_FILE, adam, best_w, best_val_brier,
...
def main():
    parser = argparse.ArgumentParser(description="ODIN Honing Engine v5.2 (Fast CRS+SWP + Better Selection)")
    ...
    parser.add_argument('--out_prefix', type=str, default=None,
                        help='If set (e.g., v1250), writes odin_<prefix>_{honed,ensemble,best,anchor}.json')
    parser.add_argument('--out_honed', type=str, default=None, help='Override honed output path')
    parser.add_argument('--out_ensemble', type=str, default=None, help='Override ensemble output path')
    parser.add_argument('--out_best', type=str, default=None, help='Override selected-best output path')
    parser.add_argument('--out_anchor', type=str, default=None, help='Write next anchor to this path')
    parser.add_argument('--update_anchor', action='store_true',
                        help='Overwrite DEFAULT_ANCHOR with selected best (for perpetual mode)')
    args = parser.parse_args()

    print(f"""
╔══════════════════════════════════════════════════════════════╗
║  ODIN HONING ENGINE v5.2 — Fast CRS+SWP + Better Selection                 ║
║  Fast CRS+SWP + WF-regularized search (54 weights)            ║
╠══════════════════════════════════════════════════════════════╣
