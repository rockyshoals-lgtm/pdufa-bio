#!/usr/bin/env python3
"""
ODIN GOD MODE V5 — GPU CONFIG SEARCH ENGINE (RTX 4070 / CUDA) - CORRECTED V2
-----------------------------------------------------------------------------
CHANGES FROM V1:
  1. Fixed FP normalization bug (was /25, now dynamic /n_crl)
  2. Added Excel (.xlsx) file support
  3. Added --recall_min constraint to prevent over-conservative models
  4. Changed objective to balance Brier, F1, and Specificity
  5. Added recall tracking in output

Inputs:
  - ODIN_PDUFA_1349_GPU_READY.csv (or .xlsx or .npz)
...
class ODINRTXV5Engine:
    """GPU search engine - CORRECTED VERSION V2 with recall constraint."""
    ...
    def _objective(self):
        ...
        recall = self.out_tp / (self.out_tp + self.out_fn + 1e-9)
        ...
        # FP penalty normalized by actual CRL count
        fp_penalty = 1.0 - cp.minimum(fpc / float(self.n_crl), 1.0)
        
        # NEW OBJECTIVE: Balance Brier (calibration), F1 (precision+recall), specificity, and FP
        # 40% Brier, 30% F1, 20% Specificity, 10% FP penalty
        obj = ((1.0 - b) * 0.40) + (f * 0.30) + (s * 0.20) + (fp_penalty * 0.10)
        return idx, obj
    ...
    def save(self, out_json: str) -> None:
        print("\n🏆 TOP CONFIGS:")
        print("-" * 100)
        print(f"{'#':>3} {'score':>8} {'brier':>8} {'prec':>6} {'recall':>6} {'F1':>6} {'spec':>6} {'FP':>4} {'FN':>4} {'thr':>6}")
        print("-" * 100)
        for i, r in enumerate(self.top, 1):
            m = r["metrics"]
            print(
                f"{i:>3} {r['score']:>8.5f} {m['brier']:>8.5f} {m['precision']:>6.3f} {m['recall']:>6.3f} "
                f"{m['f1']:>6.3f} {m['specificity']:>6.3f} {m['fp']:>4} {m['fn']:>4} {r['params']['p_threshold']:>6.3f}"
            )
        print("-" * 100)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data", type=str, default="ODIN_PDUFA_1349_GPU_READY.csv",
