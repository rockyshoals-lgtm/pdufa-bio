...
"""
================================================================================
GUNGNIR v36.2.0 KAIZEN — Regularization + Temperature Tuning
================================================================================
...
DEPLOY_JSON = os.path.join(DATA_DIR, "gungnir_v36_2_deploy.json")
...
def main():
    ...
    print("=" * 80)
    print("GUNGNIR v36.2.0 KAIZEN TRAINING PIPELINE")
    print("=" * 80)
    if not HAS_XGB:
    ...
    for split_name, test_start, test_end in splits:
        ...
        m1 = LogisticRegression(C=0.3, penalty="l2", solver="lbfgs", max_iter=2000)
        m1.fit(X_tr, y_train)
        p1 = m1.predict_proba(X_te)[:, 1]

        # Model 2: L2 Ridge for P(GOOD+)
        m2 = LogisticRegression(C=0.3, penalty="l2", solver="lbfgs", max_iter=2000)
    ...
    m1_final = LogisticRegression(C=0.3, penalty="l2", solver="lbfgs", max_iter=2000)
    m1_final.fit(X_full, y_bin)

    # Model 2: P(GOOD+)
    m2_final = LogisticRegression(C=0.3, penalty="l2", solver="lbfgs", max_iter=2000)
    ...
    XGB_PATH = os.path.join(DATA_DIR, "gungnir_v36_2_xgb.json")
    ...
    deploy = {
        ...
        "xgb_model_path": "gungnir_v36_2_xgb.json",
    ...
    print(f"\n{'='*80}")
    print(f"GUNGNIR v36.2.0 KAIZEN TRAINING COMPLETE")
    print(f"{'='*80}")
