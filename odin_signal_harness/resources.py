...
def _read_meminfo_windows() -> Optional[Tuple[int, int, float]]:
    """Return (total_bytes, available_bytes, used_percent) on Windows.

    Uses the Win32 GlobalMemoryStatusEx API via ctypes so we don't require
    `psutil` for dynamic RAM budgeting on Windows.
    """
    try:
        import ctypes
        ...
        used_pct = float(stat.dwMemoryLoad) / 100.0
        return total_b, avail_b, used_pct
    except Exception:
        return None
...
def get_memory_bytes() -> Dict[str, Any]:
    ...
    try:
        ...
        # Windows fallback (no /proc/meminfo).
        if os.name == "nt":
            win = _read_meminfo_windows()
            if win is not None:
                total_b, avail_b, used_pct = win
                return {
                    "total_bytes": int(total_b),
                    "available_bytes": int(avail_b),
                    "used_percent": float(used_pct),
                    "source": "ctypes:GlobalMemoryStatusEx",
                }
