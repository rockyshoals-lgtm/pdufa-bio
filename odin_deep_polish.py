#!/usr/bin/env python3
"""
ODIN "Deep Polish & Prune" Optimizer (v18.0)
============================================
- LOADS the best result from 'odin_cd_final.csv'.
- STEP 1: PRUNING. Tests setting small weights to 0.0. 
  (Keeps 0.0 if ODIN Score is maintained and Brier doesn't explode).
- STEP 2: HYPER-FINE TUNING. Runs CD with 1000 steps/param on remaining weights.
- GOAL: The simplest, most robust model with Score 1252.
"""
...
# --- 1. INITIALIZATION ---
print("--- ODIN DEEP POLISH INITIALIZING ---", flush=True)
...
DEFAULT_SEED_FILE = "odin_cd_final.csv"
OUTPUT_DIR = "deep_polish_results"
FP_PENALTY = 2.0

# Pruning Threshold: Weights smaller than this are candidates for 0.0
PRUNE_THRESHOLD = 0.02 

# Fine Tuning Settings
MAX_CYCLES = 30
SCAN_STEPS = 500  # Extreme precision
SCAN_RANGE = 0.01 # Tight range
...
def evaluate_single_config_gpu(X_gpu, y_gpu, config_values, param_names):
    """Evaluates a single config on GPU and returns (Score, Brier, T1_N)."""
    # Reshape for broadcasting (1, P)
    B = 1
    batch_vals = cp.asarray(config_values, dtype=cp.float32).reshape(1, -1)
    ...
    # Hard Avoid Logic
    # Indices for EMA CMC and CMC Extension
    keys = list(PARAM_TO_FEATURE_MAP.keys())
    i_ema = keys.index("w_ema_cmc")
    i_ext = keys.index("w_cmc_ext")
    avoid_mask = (X_gpu[:, i_ema] > 0.5) | (X_gpu[:, i_ext] > 0.5)
    
    if avoid_mask.any():
        probs[:, avoid_mask] = cp.minimum(probs[:, avoid_mask], 0.49)
        
    is_t1 = probs >= t1_cuts
    t1_n = cp.sum(is_t1)
    
    t1_tp = cp.sum(is_t1 & (y_gpu == 1))
    t1_fp = cp.sum(is_t1 & (y_gpu == 0))
    score = t1_tp - (FP_PENALTY * t1_fp)
    
    diff_sq = (probs - y_gpu) ** 2
    brier = cp.mean(diff_sq)
    
    return float(score), float(brier), int(t1_n)
...
def print_final_json(best_vals, param_names):
    print("\n" + "="*40)
    print("      FINAL DEEP POLISHED CONFIG      ")
    print("="*40)
...
# --- MAIN ---
def main():
    print("3. Starting DEEP POLISH loop...", flush=True)
    ...
    elif os.path.exists("cd_results/" + DEFAULT_SEED_FILE):
        seed_df = pd.read_csv("cd_results/" + DEFAULT_SEED_FILE)
    else:
    ...
    # 1. EVAL BASELINE
    base_score, base_brier, base_n = evaluate_single_config_gpu(X_gpu, y_gpu, current_config, param_names)
    print(f"\n[BASELINE] Score: {base_score:.1f} | Brier: {base_brier:.6f} | T1 Count: {base_n}")
    
    # --- PHASE 1: PRUNING ---
    print("\n--- PHASE 1: PRUNING SMALL WEIGHTS ---")
    print(f"Checking weights with absolute value < {PRUNE_THRESHOLD}...")
    
    # Identify candidates
    prune_candidates = []
    for i, p_name in enumerate(param_names):
        if p_name in NON_WEIGHT_PARAMS: continue
        if abs(current_config[i]) < PRUNE_THRESHOLD:
            prune_candidates.append(i)
            
    print(f"Found {len(prune_candidates)} candidates for pruning.")
    
    for idx in prune_candidates:
        p_name = param_names[idx]
        original_val = current_config[idx]
        
        # Try setting to 0.0
        test_config = current_config.copy()
        test_config[idx] = 0.0
        
        s, b, n = evaluate_single_config_gpu(X_gpu, y_gpu, test_config, param_names)
        
        # Valid if Score is same/better AND Brier is not significantly worse
        # We allow Brier to degrade slightly (e.g. 0.00001) for the sake of Sparsity
        if n >= 1200 and n <= 2000 and s >= base_score:
            if b <= base_brier + 0.00001:
                print(f"   [PRUNED] {p_name}: {original_val:.5f} -> 0.0 (Score: {s}, Brier: {b:.6f})")
                current_config[idx] = 0.0
                base_score, base_brier = s, b
            else:
                print(f"   [KEEP]   {p_name}: 0.0 hurt confidence (Brier: {b:.6f} vs {base_brier:.6f})")
        else:
            print(f"   [KEEP]   {p_name}: 0.0 hurt score/count (Score: {s}, N: {n})")

    # --- PHASE 2: HYPER-FINE TUNING ---
    print("\n--- PHASE 2: HYPER-FINE COORDINATE DESCENT ---")
    current_scan_range = SCAN_RANGE
    ...
    for cycle in range(MAX_CYCLES):
        ...
        improved = False
        if new_score > best_score: improved = True
        elif new_score == best_score and new_brier < best_brier: improved = True
            
        if improved:
            best_score = new_score
            best_brier = new_brier
            current_config = cp.asnumpy(batch_vals[best_global_idx])
            print(f"Cycle {cycle+1}: Improved! Score: {best_score:.1f} | Brier: {best_brier:.7f}")
        else:
            print(f"Cycle {cycle+1}: No change.")
            current_scan_range *= 0.8
            if current_scan_range < 0.0001:
                print("   -> Converged (Range too small).")
                break
            print(f"   -> Reducing range to {current_scan_range:.5f}")

    out_path = os.path.join(OUTPUT_DIR, "odin_deep_polish_final.csv")
