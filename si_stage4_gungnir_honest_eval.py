#!/usr/bin/env python3
"""
Phase 2 Stage 4b — Honest eval: Short-interest features on Gungnir v48.

Methodology (STRICT — matches ODIN v18 SI eval pattern, 3-way split):
  ...
  - C swept on val only
  - Candidate: baseline + SI feature (one at a time), val gate Δval_AUC >= +0.002
...
Input:
  ...
  - si_event_features.csv (ticker + catalyst_date + short-interest features)

Output:
  - si_gungnir_honest_results.json
"""
...
SI_CSV = BASE / 'si_event_features.csv'
OUT_JSON = BASE / 'si_gungnir_honest_results.json'
...
def load_si_features():
    """Load SI event feature CSV. Key = (ticker, catalyst_date_str)."""
    d = {}
    feature_cols = []
    if not SI_CSV.exists():
        print(f'WARNING: {SI_CSV} not found — SI stage 3 must complete first')
        return d, feature_cols
    with open(SI_CSV) as f:
        reader = csv.DictReader(f)
        for r in reader:
            if not feature_cols:
                feature_cols = [c for c in reader.fieldnames if c.startswith('si_')]
...
def main():
    ...
    # Load SI features
    print('[+] Loading SI features')
    si_map, si_cols = load_si_features()
    print(f'    {len(si_map):,} events with SI data, {len(si_cols)} feature columns')

    if not si_cols:
        print('ERROR: No SI feature columns found. Skipping greedy phase — baseline only.')
        X_si = np.zeros((len(events), 0))
        n_matched = 0
    else:
        n_matched = 0
        X_si = np.zeros((len(events), len(si_cols)))
        for i, (tk, cd) in enumerate(event_keys):
            ...
            if key in si_map:
                n_matched += 1
                for j, c in enumerate(si_cols):
                    X_si[i, j] = si_map[key].get(c, 0.0)
    ...
    # Greedy forward over SI features
    selected = []
    selected_idx = []
    greedy_log = []
    ...
    if si_cols:
        print(f'[+] Greedy forward selection over {len(si_cols)} SI features (gate Δval >= {VAL_GATE})')
        for round_num in range(1, MAX_GREEDY_ROUNDS + 1):
            ...
            for j, cname in enumerate(si_cols):
                if j in selected_idx:
                    continue
                cols = selected_idx + [j]
                X_tr = np.hstack([X_base[train_idx], X_si[train_idx][:, cols]])
                X_v = np.hstack([X_base[val_idx], X_si[val_idx][:, cols]])
                X_te = np.hstack([X_base[test_idx], X_si[test_idx][:, cols]])
                try:
    ...
    # Final model
    if selected_idx:
        X_tr = np.hstack([X_base[train_idx], X_si[train_idx][:, selected_idx]])
        X_v = np.hstack([X_base[val_idx], X_si[val_idx][:, selected_idx]])
        X_te = np.hstack([X_base[test_idx], X_si[test_idx][:, selected_idx]])
    ...
    results = {
        ...
        'data': {
            ...
            'n_si_candidate_features': len(si_cols),
            'n_gungnir_events_matched_with_si': int(n_matched),
            'si_coverage_pct': float(100.0 * n_matched / max(1, len(events))),
        },
