#!/usr/bin/env python3
"""
Phase 2 Stage 4 — Honest eval: Short Interest features on ODIN v18.
...
Input:
  ...
  - si_event_features.csv (ticker + catalyst_date + 23 SI features, T-1 compliant)

Output:
  - si_odin_honest_results.json (methodology + baseline + candidate AUCs + lift CI + greedy log + final test AUC)
"""
...
OUT_JSON = BASE / 'si_odin_honest_results.json'
...
def load_si_features():
    ...
    if not SI_CSV.exists():
        print(f'WARNING: {SI_CSV} not found')
...
def main():
    ...
    print(f'    {len(si_map):,} SI events, {len(si_cols)} feature columns')

    # Attach SI features per row (default 0 if unmatched)
    n_matched = 0
    X_si = np.zeros((len(df), len(si_cols)))
    ...
    # Greedy forward selection over SI features
    print(f'[+] Greedy forward selection over {len(si_cols)} SI features')
    selected = []
    selected_idx = []
    greedy_log = []
    ...
    results = {
        ...
        'data': {
            ...
            'si_coverage_pct': 100 * n_matched / max(1, len(df)),
        },
