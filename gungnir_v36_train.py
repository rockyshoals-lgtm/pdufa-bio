...
"""
================================================================================
GUNGNIR v36.1.0 KAIZEN — Regularization + Temperature Tuning
================================================================================
...
def main():
    ...
    print("=" * 80)
    print("GUNGNIR v36.1.0 KAIZEN TRAINING PIPELINE")
    print("=" * 80)
    if not HAS_XGB:
    ...
    print("\n[ENGINEER] Building v33 feature vectors...")
    feature_names = None
    X_rows = []
    ...
    for split_name, test_start, test_end in splits:
        ...
        # Model 1: L2 Ridge for P(positive) — v36.1 KAIZEN: C=0.10
        m1 = LogisticRegression(C=0.10, penalty="l2", solver="lbfgs", max_iter=2000)
        m1.fit(X_tr, y_train)
        ...
        # Model 3: L2 Ridge for P(CRASH) — v36.1 R4: C=0.3 (tighter crash model)
        m3 = LogisticRegression(C=0.3, penalty="l2", solver="lbfgs", max_iter=2000)
        m3.fit(X_tr, y_cr_train)
        p3 = m3.predict_proba(X_te)[:, 1]

        # Model 4: DROPPED — ElasticNet provides zero lift per R4 grid search
        # (meta-weight = 0, keeping code for reference but not in blend)

        # Model 5: XGBoost for P(positive) — v36.1 R4: slower learning for stability
        ...
        # v36.1 R4: 60/0/40 Ridge+XGBoost (ElasticNet dropped)
        p_meta = 0.60 * p1 + 0.40 * p5
        p_meta = np.clip(p_meta, 0.02, 0.98)

        # Temperature scaling — v36.1 R4: T=1.0 (no scaling, best Brier)
        base_rate = y_train.mean()
        logits = np.log(p_meta / (1 - p_meta))
        T = 1.00  # v36.1 R4: no temperature scaling (best calibration)
    ...
    m1_final = LogisticRegression(C=0.10, penalty="l2", solver="lbfgs", max_iter=2000)
    ...
    # Model 3: P(CRASH) — v36.1 R4: C=0.3
    m3_final = LogisticRegression(C=0.3, penalty="l2", solver="lbfgs", max_iter=2000)
    m3_final.fit(X_full, y_cr)

    # Model 4: ElasticNet — DROPPED (zero lift per R4 grid search)
    # Keeping training for deploy JSON compatibility but weight=0
    ...
    # Model 5: XGBoost — v36.1 R4: lr=0.03 (slower, more stable)
    ...
    XGB_PATH = os.path.join(DATA_DIR, "gungnir_v36_xgb.json")
    ...
    deploy = {
        "version": "36.1.0",
        "codename": "Allfather_Ascendant",
        "architecture": "5-model meta-ensemble (Ridge_Binary 50% + ElasticNet 20% + XGBoost 30%) + Ridge_GOOD+ + Ridge_CRASH + Bayesian strata + T=0.85",
        "meta_weights": {"ridge_binary": 0.60, "elasticnet": 0.00, "xgboost": 0.40},
        "xgb_model_path": "gungnir_v36_xgb.json",
    ...
    print(f"\n{'='*80}")
    print(f"GUNGNIR v36.1.0 KAIZEN TRAINING COMPLETE")
    print(f"{'='*80}")
    print(f"  Architecture: 5-model meta-ensemble (Ridge 50% + EN 20% + XGB 30%) + Bayesian strata + T=0.85")
    print(f"  Features: {len(feature_names)}")
