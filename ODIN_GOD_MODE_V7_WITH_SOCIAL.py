#!/usr/bin/env python3
"""
ODIN GOD MODE V7 — GPU CONFIG SEARCH ENGINE WITH SOCIAL MEDIA SENTIMENT
--------------------------------------------------------------------------------
NEW IN V7 (SOCIAL SIGNALS):
  - Integrated LunarCrush social media sentiment signals (S17-S20)
  - S17: Social Sentiment (+0.03 if sentiment ≥75%, -0.02 if ≤40%)
  - S18: Engagement Spike (+0.02 if 2x avg engagement with bullish sentiment)
  - S19: Social Silence (-0.01 if engagement <30% of average)
  - S20: Smart Money Divergence (-0.02 if galaxy <40 & sentiment <50)
  - Added w_social weight parameter to control social signal contribution
  - social_total column used directly (pre-computed S17+S18+S19+S20)

T-1 COMPLIANCE (SOCIAL SIGNALS):
  Social signals are T-1 compliant because they capture market sentiment
  BEFORE the PDUFA decision date. LunarCrush data reflects pre-decision
  social media activity, investor positioning, and market expectations.

SOCIAL SIGNAL ANALYSIS (from LunarCrush enrichment):
  ...
  - Coverage: 79.4% of events have non-zero social signals

INHERITED FROM V6:
  - T-1 compliant manufacturing_risk (modality-based)
  - Fixed FP normalization bug (dynamic /n_crl)
  - Excel (.xlsx) file support
  - --recall_min constraint
  - Multiple objective modes (balanced, avoid_fp, spec_at_prec, calibration)

Inputs:
  - ODIN_PDUFA_1925_ENRICHED_WITH_SOCIAL.csv (with social columns)
...
# --------------------------
# FUSED CUDA KERNEL WITH SOCIAL SIGNALS
# --------------------------
...
void fused_score_kernel(
    ...
    const float* __restrict__ social_total,  // NEW: social signal (S17+S18+S19+S20)
    ...
    const float* __restrict__ w_social,  // NEW: social weight
    ...
    float pw_social   = w_social[config_idx];  // NEW
    ...
    for (int i = tid; i < N; i += blockDim.x) {
        ...
        if (class1_cmc[i] == 1) {
            ...
            // NEW: Social signals (pre-computed total from S17+S18+S19+S20)
            // social_total ranges from -0.04 to +0.05
            // w_social amplifies this contribution
            // Positive social = bullish sentiment = higher approval probability
            // Negative social = bearish/silent = lower approval probability
...
class ODINRTXV7Engine:
    """GPU search engine - V7 with SOCIAL MEDIA SENTIMENT SIGNALS."""
    ...
    def _default_bounds(self) -> Dict[str, Bounds]:
        return {
            # Base and threshold
            "p_base": Bounds(0.70, 0.95),
            "p_threshold": Bounds(0.40, 0.90),
            ...
            "w_mfg_pen": Bounds(-0.15, -0.01),
            "w_mfg_amp": Bounds(1.0, 1.5),
            "i_mfg_inexp": Bounds(0.9, 1.3),

            # Therapeutic area adjustments
            "adj_pain": Bounds(-0.55, -0.05),
            "adj_cns": Bounds(-0.35, 0.10),
            "adj_cns_amp": Bounds(-0.45, -0.05),
            "adj_onco": Bounds(-0.02, 0.25),
            "adj_inf": Bounds(-0.02, 0.30),

            # AdCom and designation trap
            "w_adcom": Bounds(0.00, 0.40),
            "w_des_trap": Bounds(-0.25, -0.03),

            # NEW: Social signal weight
            # social_total ranges from -0.04 to +0.05
            # w_social amplifies this: 
            #   At w_social=1.0: contribution range = [-0.04, +0.05]
            #   At w_social=3.0: contribution range = [-0.12, +0.15]
            #   At w_social=5.0: contribution range = [-0.20, +0.25]
            # Analysis showed negative social correlates with 27% lower approval
            "w_social": Bounds(0.0, 5.0),
        }
    ...
    def _load_npz(self, path: str) -> Dict[str, np.ndarray]:
        ...
        needed = [
            ...
            "pain","cns","onco","inf","stack","class1_cmc","des_trap","adcom_pct",
            "social_total","split_code"
        ]
    ...
    def _load_data(self) -> None:
        ...
        if path.lower().endswith(".npz"):
            ...
            self.feat = {
                ...
                "social_total": filt(d["social_total"], np.float32),
            }
            ...
            # Load social_total (pre-computed S17+S18+S19+S20)
            social_total = get_float("social_total", 0.0)
        ...
        for name, arr in self.feat.items():
            if name == "adcom_pct":
                ...
                n_neg = (arr < 0).sum()
                n_pos = (arr > 0).sum()
                print(f"   📱 {name}: {n_nonzero} non-zero ({n_nonzero/self.N*100:.1f}%), range=[{min_val:.4f}, {max_val:.4f}], mean={mean_val:.4f}")
                print(f"       → {n_pos} bullish (+), {n_neg} bearish (-)")
            elif name == "stack":
        ...
        # Move to GPU
        self.gpu = {
            k: cp.asarray(v, dtype=(cp.float32 if k in ("adcom_pct", "social_total") else cp.int32))
    ...
    def _launch(self) -> None:
        ...
        SCORING_KERNEL(
            ...
            (
                ...
                self.gpu["adcom_pct"], self.gpu["social_total"], self.y_true_gpu,
        ...
        for bi in range(batches):
            ...
            if now - last >= report_every_s:
                ...
                if self.top:
                    ...
                    best_social = self.top[0]["params"].get("w_social", 0)
                    best_str = f"F1={best_f1:.4f}, w_social={best_social:.2f}"
                else:
                    best_str = "—"
    ...
    def save(self, out_json: str) -> None:
        print("\n🏆 TOP CONFIGS (WITH SOCIAL SIGNALS):")
        print("-" * 130)
        print(f"{'#':>3} {'score':>8} {'brier':>8} {'prec':>6} {'recall':>6} {'F1':>6} {'MCC':>6} {'spec':>6} {'FP':>4} {'FN':>4} {'thr':>6} {'w_social':>9}")
        print("-" * 130)
        ...
        # Social signal analysis across all top configs
        if self.top:
            w_social_values = [r["params"]["w_social"] for r in self.top]
            f1_values = [r["metrics"]["f1"] for r in self.top]
            
            print("\n📱 SOCIAL SIGNAL ANALYSIS (across top configs):")
            print(f"   w_social range: [{min(w_social_values):.3f}, {max(w_social_values):.3f}]")
            print(f"   w_social mean:  {np.mean(w_social_values):.3f} ± {np.std(w_social_values):.3f}")
            print(f"   w_social median: {np.median(w_social_values):.3f}")
            
            # Quartile analysis
            q1 = np.percentile(w_social_values, 25)
            q3 = np.percentile(w_social_values, 75)
            print(f"   w_social Q1/Q3: [{q1:.3f}, {q3:.3f}]")
            
            # Correlation hint
            if len(self.top) >= 10:
                # Check if higher w_social correlates with better F1
                top_10_social = np.mean([r["params"]["w_social"] for r in self.top[:10]])
                bot_10_social = np.mean([r["params"]["w_social"] for r in self.top[-10:]]) if len(self.top) >= 20 else top_10_social
                print(f"   Top-10 avg w_social: {top_10_social:.3f}")
                if len(self.top) >= 20:
                    print(f"   Bottom-10 avg w_social: {bot_10_social:.3f}")
                    if top_10_social > bot_10_social + 0.1:
                        print("   → 📈 Higher social weights correlate with better configs")
                    elif bot_10_social > top_10_social + 0.1:
                        print("   → 📉 Lower social weights correlate with better configs")
                    else:
                        print("   → ➡️ Social weight shows no strong correlation with performance")
            
            # Distribution buckets
            low_social = sum(1 for w in w_social_values if w < 1.0)
            mid_social = sum(1 for w in w_social_values if 1.0 <= w < 3.0)
            high_social = sum(1 for w in w_social_values if w >= 3.0)
            print(f"\n   Distribution: {low_social} low (<1.0), {mid_social} mid (1-3), {high_social} high (≥3.0)")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ODIN V7 GPU Config Search with Social Signals")
    p.add_argument("--data", type=str, default="ODIN_PDUFA_1925_ENRICHED_WITH_SOCIAL.csv",
                   help="Path to .csv, .xlsx, or .npz dataset with social columns.")
    ...
    p.add_argument("--iters", type=int, default=500_000_000, help="Total iterations for Phase 1.")
    p.add_argument("--iters2", type=int, default=250_000_000, help="Total iterations for Phase 2 (refinement).")
    ...
    p.add_argument("--out", type=str, default="ODIN_TOP_CONFIGS_V7_WITH_SOCIAL.json", help="Output JSON path.")
...
def main() -> None:
    ...
    print("\n" + "="*72)
    print("📱 ODIN GOD MODE V7 — WITH SOCIAL MEDIA SENTIMENT")
    print("="*72)
    print("  Social signals: S17 (sentiment), S18 (spike), S19 (silence), S20 (divergence)")
    print("  T-1 compliant: Pre-decision market sentiment from LunarCrush")

    engine = ODINRTXV7Engine(
        data_path=args.data,
    ...
    print("\n" + "="*72)
    print("⚔️  PHASE 1: GLOBAL SEARCH (WITH SOCIAL SIGNALS)")
    print("="*72)
