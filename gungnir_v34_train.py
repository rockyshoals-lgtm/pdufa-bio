...
"""
================================================================================
GUNGNIR v34.0.0 — ALLFATHER ASCENDANT PHASE READOUT PREDICTION ENGINE
================================================================================
...
DEPLOY_JSON = os.path.join(DATA_DIR, "gungnir_v34_deploy.json")
...
def engineer_v31_features(row, ctgov_lookup=None, journey_index=None):
    ...
    # --- v33 NEW: PRE-READOUT MOMENTUM (T-1 compliant) ---
    ...
    # --- v33 NEW: COMPETITIVE LANDSCAPE (T-1 compliant) ---
    ...
    # --- v34 NEW: IIS (INTERIM INFLATION SCORE) FEATURES ---
    # NOTE: smart_money_flag and iis_score_auto REMOVED — they use outcome column (LABEL LEAKAGE)
    # Only T-1 compliant features retained: interim detection, N per arm, prior readout, combined dose
    iis = row.get("_iis", {})
    features["is_interim"] = iis.get("v34_is_interim", 0)
    features["n_per_arm_log"] = iis.get("v34_n_per_arm_log", math.log(60))  # default Phase 2 median
    features["is_small_n"] = iis.get("v34_is_small_n", 0)
    features["has_prior_readout_drug"] = iis.get("v34_has_prior_readout", 0)
    features["days_since_prior_log"] = iis.get("v34_days_since_prior_log", 0)
    features["combined_dose_flag"] = iis.get("v34_combined_dose_flag", 0)

    # --- v34 IIS INTERACTIONS (T-1 CLEAN ONLY) ---
    features["interim_x_small_n"] = features["is_interim"] * features["is_small_n"]
    features["interim_x_phase2"] = features["is_interim"] * features["is_phase2"]
    features["interim_x_micro"] = features["is_interim"] * features["is_micro"]
    features["small_n_x_phase2"] = features["is_small_n"] * features["is_phase2"]
...
def main():
    ...
    print("=" * 80)
    print("GUNGNIR v34.0.0 — ALLFATHER ASCENDANT TRAINING PIPELINE")
    print("=" * 80)
    if not HAS_XGB:
        print("  ERROR: XGBoost is REQUIRED for v33. Install with: pip install xgboost")
        return 1
    ...
    # Step 4a: Load momentum cache (v33 NEW)
    print("\n[v33 NEW] Loading pre-readout momentum data...")
    ...
    if os.path.exists(MOMENTUM_CACHE):
        ...
        print("  [WARNING] Momentum cache not found — momentum features will be zero")

    # Attach momentum to events
    mom_attached = 0
    for ev in merged:
        ...
        if mom and "error" not in mom and mom.get("d_m5"):
            mom_attached += 1
    print(f"  Momentum attached: {mom_attached}/{len(merged)}")

    # Step 4b: Build competitive landscape (v33 NEW)
    print("\n[v33 NEW] Building competitive landscape index...")
    ...
    print(f"  Competitive landscape: {len(indication_dates)} indications tracked")

    # Step 4c: Load IIS features (v34 NEW)
    IIS_FEATURES = os.path.join(DATA_DIR, "iis_features_auto.json")
    iis_cache = {}
    if os.path.exists(IIS_FEATURES):
        with open(IIS_FEATURES) as f:
            iis_cache = json.load(f)
    print(f"\n[v34 NEW] Loading IIS features...")
    print(f"  IIS cache: {len(iis_cache)} entries")
    # Attach to events
    iis_matched = 0
    for ev in merged:
        key = f"{ev['ticker']}|{ev['date']}"
        iis_data = iis_cache.get(key, {})
        ev["_iis"] = iis_data
        if iis_data:
            iis_matched += 1
    print(f"  IIS matched: {iis_matched}/{len(merged)}")

    # Step 5: Engineer features
    print("\n[ENGINEER] Building v34 feature vectors...")
    feature_names = None
    X_rows = []
    ...
    for split_name, test_start, test_end in splits:
        ...
        # Model 5: XGBoost for P(positive) — v33 NEW: gradient boosting diversity
        ...
        # v33: 50/20/30 Ridge+ElasticNet+XGBoost blend
    ...
    print("\n[TRAIN] Training final v33 models on full dataset...")
    scaler = StandardScaler()
    ...
    # Save XGBoost model separately (not JSON-serializable as coefficients)
    XGB_PATH = os.path.join(DATA_DIR, "gungnir_v34_xgb.json")
    ...
    deploy = {
        "version": "33.0.0",
        "codename": "Allfather_IIS",
        "architecture": "5-model meta-ensemble (Ridge_Binary 50% + ElasticNet 20% + XGBoost 30%) + Ridge_GOOD+ + Ridge_CRASH + Bayesian strata + T=0.85 + IIS features",
        "meta_weights": {"ridge_binary": 0.50, "elasticnet": 0.20, "xgboost": 0.30},
        "xgb_model_path": "gungnir_v34_xgb.json",
    ...
    print(f"\n{'='*80}")
    print(f"GUNGNIR v34.0.0 TRAINING COMPLETE")
    print(f"{'='*80}")
    print(f"  Architecture: 5-model meta-ensemble (Ridge 50% + EN 20% + XGB 30%) + Bayesian strata + T=0.85 + IIS")
    print(f"  Features: {len(feature_names)}")
