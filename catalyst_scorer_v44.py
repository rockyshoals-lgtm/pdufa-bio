#!/usr/bin/env python3
"""
CATALYST SCORER v44.0.0 + IIS v1.0 — Full Enriched Scoring Pipeline
====================================================================
Scores 2026 catalysts through the complete Gungnir v44 enriched pipeline:
  - 146 features (v33 base + v37-v44 interaction chains)
...
DEPLOY_JSON = os.path.join(DATA_DIR, "gungnir_v44_deploy.json")
...
XLSX_PATH = os.path.join("/sessions/loving-nifty-dirac/mnt/uploads", "fda_2026-04-08 (1).xlsx")
OUTPUT_JSON = os.path.join(DATA_DIR, "catalyst_scores_v44.json")
...
def engineer_v44_features(event, ctgov_data=None, ch2_features=None):
    ...
    # --- v44 CROSS-FAMILY INTERACTION FEATURES ---
    # Feature 1: Antagonist mechanism × prior positive readout
    # Antagonist drugs (receptor blockers) that already showed positive data succeed more
    moa_antagonist = ch2.get("ch2_moa_antagonist", 0) if ch2 else 0
    features["v44_ch2_moa_antagonist_X_journey_had_positive"] = moa_antagonist * features["journey_had_positive"]

    # Feature 2: Small molecule × Phase 2 × small-cap (three-way)
    # Focused small-cap bets on cheaper drugs at lower regulatory bar = asymmetric upside
    ch2_is_sm = ch2.get("ch2_is_sm", 0) if ch2 else 0
    features["v44_ch2_is_sm_X_is_phase2_X_is_small"] = ch2_is_sm * features.get("is_phase2", 0) * features["is_small"]
...
def main():
    print("=" * 80)
    print("CATALYST SCORER v44.0.0 + IIS v1.0 — Full Enriched Pipeline")
    print("=" * 80)
    ...
    # Score all events
    print(f"\n[SCORE] Scoring {len(events)} catalysts with Gungnir v44 enriched pipeline...")
    scored = []
    ctgov_matched = 0
    for event in events:
        ctgov_data = ctgov_cache.get(event["nct_id"], {})
        ...
        features = engineer_v44_features(event, ctgov_data, event.get("_ch2"))
    ...
    # Summary
    print(f"\n{'='*80}")
    print("v44.0.0 ENRICHED INVESTMENT TIER SUMMARY")
    print(f"{'='*80}")
