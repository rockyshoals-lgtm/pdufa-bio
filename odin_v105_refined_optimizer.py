#!/usr/bin/env python3
"""
ODIN v10.5 "Refined" Liquid Optimizer
=====================================
- ALGORITHM: Gaussian Evolution (Exploration -> Exploitation -> Polishing).
- OBJECTIVE: Maximizes Separation (AUC) & Minimizes Brier Score.
- REGULARIZATION: Penalizes excessively large weights to prevent overfitting.
- ROBUSTNESS: Includes all previous data-type safety fixes.
"""
...
# Search Parameters
BATCH_SIZE = 100_000
N_BATCHES = 200        # Increased to 200 for better convergence
TARGET_TIER1_PCT = 0.20 
REGULARIZATION = 0.005 # Penalty for large weights (keeps model simple)
...
class OdinLiquidEngine:
    ...
    def optimize_batch(self, batch_weights):
        ...
        score_pos = cp.sum(probs * self.y, axis=1) / max(y_sum, 1.0)
        score_neg = cp.sum(probs * (1-self.y), axis=1) / max(neg_sum, 1.0)
        separation = score_pos - score_neg
        
        # 6. Regularization (L2 on weights)
        # We ignore column 0 (Base rate) for regularization
        w_penalty = cp.mean(batch_weights[:, 1:]**2, axis=1) * REGULARIZATION
        
        # Final Fitness
        fitness = separation - (2.0 * brier) - w_penalty
...
def main():
    ...
    # Init Means with neutral values
    current_means = np.zeros(len(PARAM_KEYS), dtype=np.float32)
    current_means[0] = 0.70 # Default base rate
    
    print(f"Starting EVOLUTIONARY search ({N_BATCHES} batches x {BATCH_SIZE})...")
    
    for i in range(N_BATCHES):
        # --- EVOLUTIONARY STRATEGY ---
        # Phase 1: Exploration (First 10%) - Wide Sigma
        if i < (N_BATCHES * 0.1):
            sigma = 0.50
            # Re-center periodically to 0 to keep exploring
            batch_means = current_means if i % 2 == 0 else np.zeros_like(current_means)
            if i % 2 != 0: batch_means[0] = 0.70 # Keep base reasonable
            
        # Phase 2: Exploitation (Next 50%) - Medium Sigma
        elif i < (N_BATCHES * 0.6):
            sigma = 0.15
            batch_means = current_means
            
        # Phase 3: Polishing (Last 40%) - Tight Sigma
        else:
            sigma = 0.05
            batch_means = current_means

        # Generate Gaussian Batch
        noise = np.random.standard_normal((BATCH_SIZE, len(PARAM_KEYS))).astype(np.float32)
        W = batch_means + (noise * sigma)
        ...
        W[:, pos] = np.abs(W[:, pos])
        W[:, neg] = -np.abs(W[:, neg])
        W[:, 0] = np.clip(W[:, 0], 0.50, 0.95) # Base rate clamp
        ...
        if fitness[idx] > best_fit:
            ...
            # Update the evolutionary center
            current_means = best_w
            
            phase = "Explore" if sigma > 0.4 else ("Exploit" if sigma > 0.1 else "Polish")
            print(f"Batch {i} [{phase}]: New Best! Fit={best_fit:.4f} | Brier={best_brier:.4f}")
    ...
    try:
        ...
        with open(CALIBRATOR_FILE, 'wb') as f: pickle.dump(iso, f)
