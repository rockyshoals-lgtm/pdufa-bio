"""
ODIN v10.6 "LOCAL SEARCH" REFINER
=================================
GOAL:   Hone the "Canonical" config by searching locally (+/- 20%).
LOGIC:  Logistic Regression (Sigmoid) on GPU.
HONEST: Strict Time-Series Split. Filters on Train, Validates on Test.
FIXES:  Includes all previous data loading and scalar type fixes.
"""
...
# --- THE CANONICAL BASELINE (From your previous run) ---
CANONICAL_CONFIG = {
...
# Determine signs to enforce during perturbation
PARAM_SIGNS = np.sign(CANONICAL_VEC)
PARAM_SIGNS[PARAM_SIGNS == 0] = 1.0 # Default scalars to positive if 0
...
def generate_local_configs(n):
    ...
    New_Val = Canonical_Val * Random(1-Radius, 1+Radius)
    Preserves Signs automatically.
    """
    canon_mags = cp.abs(CANONICAL_VEC)
    
    # Random factor centered on 1.0
...
def main():
    ...
    base_train_brier, base_train_sep = engine.evaluate_batch(baseline_vec, False)
    base_test_brier, base_test_sep = engine.evaluate_batch(baseline_vec, True)
    
    # Safe scalar extraction
    base_tb = float(base_train_brier.item()) if GPU_AVAILABLE else float(base_train_brier)
    base_ts = float(base_train_sep.item()) if GPU_AVAILABLE else float(base_train_sep)
    base_test_b = float(base_test_brier.item()) if GPU_AVAILABLE else float(base_test_brier)
    base_test_s = float(base_test_sep.item()) if GPU_AVAILABLE else float(base_test_sep)
    ...
    for i in range(N_BATCHES):
        ...
        # 2. Honest Filter: Must be at least as good as baseline on Train (plus tiny epsilon)
        # We allow a tiny degradation (0.002) if it helps generalization, but generally strict.
        valid_mask = (brier_train <= (base_tb + 0.002)) & (sep_train >= (base_ts - 0.01))
        
        if not valid_mask.any(): continue
        
        # 3. Select Survivors (Top 100 by Train Brier)
        survivor_indices = cp.where(valid_mask)[0]
        if len(survivor_indices) > 100:
            survivor_briers = brier_train[survivor_indices]
            sorted_args = cp.argsort(survivor_briers)[:100]
            survivor_indices = survivor_indices[sorted_args]
            
        survivor_configs = configs[survivor_indices]
        
        # 4. Check Test Performance (For Selection of Final "Best")
        test_briers, test_seps = engine.evaluate_batch(survivor_configs, use_test_set=True)
        
        # Find best Test Brier among survivors
        min_idx = cp.argmin(test_briers)
        min_test_val = float(test_briers[min_idx].item()) if GPU_AVAILABLE else float(test_briers[min_idx])
        
        if min_test_val < best_test_brier:
            best_test_brier = min_test_val
            best_config_vec = survivor_configs[min_idx]
            
            # Logging
            matching_train_brier = float(brier_train[survivor_indices[min_idx]].item()) if GPU_AVAILABLE else float(brier_train[survivor_indices[min_idx]])
            print(f"Batch {i}: NEW BEST! Test Brier: {min_test_val:.5f} (Train: {matching_train_brier:.5f})")
