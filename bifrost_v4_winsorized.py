...
"""
================================================================================
BIFROST v4 WINSORIZED RETRAIN — Apr 19, 2026
================================================================================

Retroactive retrain of the v4 kaizen pipeline with winsorized returns at
[-50%, +100%] to neutralize penny-stock outliers. Mirrors exact v4 feature
engineering and backtest methodology; ONLY the return cap changes.

Three modes tested per Kelly/LGB config (12 total configs):
  - NONE:        baseline re-run (control). Should reproduce deployed v4 Sharpe 5.45.
  - TRAIN_ONLY:  training labels clipped; backtest returns kept raw. Tests whether
                 robust training improves model quality w/o distorting reality.
  - FULL:        both training labels AND backtest returns clipped. Pessimistic
                 realistic ceiling if you actually had to cap exits at +100%.

Ship rule: v4.1 WINSORIZED = winner if Sharpe ≥ v4.0 Sharpe (5.45) AND |max_DD| ≤
|v4.0 max_DD| (4.9%). Otherwise log finding, ship nothing, keep v4 deployed.
"""
...
OUTPUT_JSON = os.path.join(DATA_DIR, "bifrost_v4_winsorized_results.json")
...
def winsorize(x, mode):
    """Clip percentage return to [LO, HI] if mode != 'none'."""
    if mode == "none":
        return x
    return max(WINSOR_LO, min(WINSOR_HI, x))
...
def build_training_data_v4(events, price_cache, train_winsor_mode):
    """Build feature matrix with v4 enhancements.

    train_winsor_mode:
      'none'        -> raw returns as training target
      'train_only'  -> training target is clipped [-50, 100]; raw retained for backtest
      'full'        -> training target clipped AND raw target also clipped
    """
    rows = []
    ...
    for ev in events:
        ...
        for win_name, entry_td, exit_td in WINDOWS:
            ...
            if not ret_str:
                continue
            try:
                raw_return = float(ret_str) * 100
            except:
                continue

            # Training target (what model is fit against):
            if train_winsor_mode in ("train_only", "full"):
                train_target = winsorize(raw_return, "clip")
            else:
                train_target = raw_return

            # Backtest target (what portfolio P&L uses):
            if train_winsor_mode == "full":
                exit_target = winsorize(raw_return, "clip")
            else:
                exit_target = raw_return
            ...
            row = {
                ...
                "_target_return": train_target,   # training label
                "_exit_return": exit_target,      # backtest P&L realization
                "_raw_return": raw_return,
...
def walk_forward_backtest(rows, feature_names, kelly_frac=0.5, use_lgb=False):
    ...
    y_all = np.array([r["_target_return"] for r in rows_sorted])  # training uses _target_return
    ...
    for evt_idx, evt_key in enumerate(event_order):
        ...
        best_entry_td = None
        best_exit_td = None
        best_return = None  # realized exit return for P&L
        best_raw = None
        ...
        for row in evt_rows:
            ...
            if pred > best_pred:
                ...
                best_return = row["_exit_return"]  # use _exit_return (winsorized or raw per mode)
                best_raw = row["_raw_return"]
    ...
    raws = [t.get("raw_return", t["return"]) for t in all_trades]
    win_rate = np.mean([r > 0 for r in returns])
    avg_return = np.mean(returns)
    avg_raw = np.mean(raws)
...
def main():
    print("="*80)
    print("BIFROST v4 WINSORIZED RETRAIN — Apr 19, 2026")
    print("="*80)
    ...
    for mode in ["none", "train_only", "full"]:
        print(f"\n{'='*80}\n[MODE] {mode.upper()}\n{'='*80}")
        rows, feature_names = build_training_data_v4(events, price_cache, mode)
        print(f"  Rows: {len(rows)}, Features: {len(feature_names)}")

        # Quick diagnostic: how many training targets were clipped
        n_clipped_high = sum(1 for r in rows if r["_raw_return"] > 100)
        n_clipped_low = sum(1 for r in rows if r["_raw_return"] < -50)
        print(f"  Training targets > 100: {n_clipped_high} | < -50: {n_clipped_low}")
        ...
        for kelly in [0.5, 0.67]:
            for lgb in [False, True]:
                ...
                if result:
                    ...
                    print(f"Sharpe={result['sharpe']:.2f}, WR={result['win_rate']:.1%}, DD={result['max_drawdown']:.1f}%, FV=${result['final_value']/1e6:.2f}M")
                else:
                    print("FAIL")
    ...
    print("\n" + "="*80)
    print("CROSS-MODE COMPARISON TABLE")
    print("="*80)
    print(f"{'Mode':<12}{'Kelly':<7}{'LGB':<6}{'Trades':<8}{'WinRate':<10}{'Sharpe':<9}{'MaxDD':<9}{'FinalValue':<12}")
    print("-"*80)
    for mode in ["none", "train_only", "full"]:
        for r in all_results[mode]:
            print(f"{mode:<12}{r['kelly']:<7.2f}{str(r['use_lgb']):<6}{r['n_trades']:<8}{r['win_rate']:<10.1%}{r['sharpe']:<9.2f}{r['max_drawdown']:<9.1f}${r['final_value']/1e6:<11.2f}M")

    print("\n" + "="*80)
    print("v4.0 DEPLOYED BASELINE (from CLAUDE.md):")
    print("  Sharpe=5.45, WinRate=70.8%, MaxDD=-4.9%, FinalValue=$18.1M, Trades=910")
    print("="*80)
    ...
    v4_dd_abs = 4.9
    verdicts = {}
    for mode in ["train_only", "full"]:
        best = max(all_results[mode], key=lambda r: r["sharpe"])
        sharpe_delta = best["sharpe"] - v4_sharpe
        dd_improved = abs(best["max_drawdown"]) <= v4_dd_abs
        if best["sharpe"] >= v4_sharpe and dd_improved:
            verdict = "SHIP as v4.1 WINSORIZED"
        elif best["sharpe"] >= v4_sharpe:
            verdict = "TIES Sharpe but DD worse - log finding, keep v4"
        elif sharpe_delta > -0.3:
            verdict = "Within noise of v4 - log finding, keep v4"
        else:
            verdict = "LOSES to v4 - keep v4 deployed"
        verdicts[mode] = {
            "best_config": f"Kelly={best['kelly']}, LGB={best['use_lgb']}",
            "sharpe": best["sharpe"],
            "sharpe_delta_vs_v4": round(sharpe_delta, 2),
    ...
    final = {
        "version": "v4_winsorized_comparison",
        "date_utc": "2026-04-19",
        "winsor_bounds": [WINSOR_LO, WINSOR_HI],
        "baseline_v4_deployed": {
            "sharpe": 5.45, "win_rate": 0.708, "max_drawdown": -4.9, "final_value": 18100000, "trades": 910,
        },
        "results_by_mode": all_results,
        "verdicts": verdicts,
        "methodology": {
            "none": "Control re-run. Raw returns for both training targets and backtest P&L.",
            "train_only": "Training targets clipped to [-50, 100]%; backtest P&L uses raw returns (reality).",
            "full": "Training targets AND backtest P&L clipped to [-50, 100]%. Pessimistic realistic ceiling.",
        },
    }
