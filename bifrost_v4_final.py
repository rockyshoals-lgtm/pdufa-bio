#!/usr/bin/env python3
"""
BIFROST v4.0.0 — FINAL CHAMPION BUILD
Focus: LGB ensemble (already showing +0.10 Sharpe over v3.1)
"""
...
V4_DEPLOY = os.path.join(DATA_DIR, "bifrost_v4_deploy.json")
...
def drawdown_governor(equity_curve):
    ...
    if current_dd >= 0.15:
        ...
        return max(0.2, 1.0 - current_dd / 0.15)
    return 1.0


def walk_forward_backtest_final(rows, feature_names):
    ...
    try:
        ...
        import subprocess
        subprocess.run(["pip", "install", "xgboost", "lightgbm", "--break-system-packages", "-q"], capture_output=True)
        import xgboost as xgb_lib
        import lightgbm as lgb_lib
    ...
    print("\n[BACKTEST] Walk-forward with Ridge/XGB/LGB ensemble...")
    ...
    for evt_idx, evt_key in enumerate(event_order):
        ...
        if next_retrain_idx < len(split_boundaries) and pdufa_date >= split_boundaries[next_retrain_idx]:
            ...
            if n_train >= 500:
                ...
                current_ridge = Ridge(alpha=100.0)
                current_ridge.fit(X_tr, y_train)

                current_xgb = xgb_lib.XGBRegressor(
                    n_estimators=300, max_depth=3, learning_rate=0.02,
                    subsample=0.8, colsample_bytree=0.7, reg_lambda=2.0, reg_alpha=0.3,
                    min_child_weight=10, random_state=42, verbosity=0
                )
                current_xgb.fit(X_tr, y_train)

                current_lgb = lgb_lib.LGBMRegressor(
                    n_estimators=300, max_depth=4, learning_rate=0.02,
                    num_leaves=31, reg_lambda=2.0, reg_alpha=0.3,
                    random_state=42, verbosity=-1
                )
                current_lgb.fit(X_tr, y_train)

                current_scaler = scaler
                print(f"  Retrain at {cutoff}: {n_train} rows")
            next_retrain_idx += 1
        ...
        kf = kelly_fraction_sweep(best_pred, 15.0, 0.5)
        ...
        allowed, _, adj_size = portfolio_heat_check(active_positions, {"mcap": mcap_tier, "size_pct": pos_pct})
        if not allowed:
            continue
...
def main():
    print("="*80)
    print("BIFROST v4.0.0 — FINAL CHAMPION")
    print("="*80)
    ...
    print("\n[BUILD] Training data v4...")
    rows, feature_names = build_training_data_v4(events, price_cache)
    print(f"  Rows: {len(rows)}, Features: {len(feature_names)}")

    results = walk_forward_backtest_final(rows, feature_names)

    final_results = {
        "version": "4.0.0",
        "architecture": "Ridge_30_XGB_35_LGB_35_magnitude_regression",
        "methodology": "walk_forward_honest",
        "improvements_over_v31": [
            "New feature: sponsor_success_rate tracking",
            "New feature: vol_adjusted_confidence",
            "New features: ta_risk interactions",
            "LightGBM ensemble (35%) + XGB (35%) + Ridge (30%)",
            "Enhanced magnitude prediction",
        ],
    ...
    print(f"\n[RESULTS SAVED] {V4_RESULTS}")

    # Summary
    print("\n" + "="*80)
    print("BIFROST v4.0.0 vs v3.1.0")
    print("="*80)
    b = final_results["baseline_v31"]
    r = results
    print(f"  Metric                  v3.1 (baseline)    v4.0 (optimized)")
    print(f"  {'-'*80}")
    print(f"  Trades                  {b['n_trades']:<18d} {r['n_trades']:<18d}")
    print(f"  Win rate                {b['win_rate']:<18.1%} {r['win_rate']:<18.1%}")
    print(f"  Avg return/trade        {b['avg_return']:<18.2f}% {r['avg_return']:<18.2f}%")
    print(f"  Sharpe ratio            {b['sharpe']:<18.2f} {r['sharpe']:<18.2f}")
    print(f"  Max drawdown            {b['max_drawdown']:<18.1f}% {r['max_drawdown']:<18.1f}%")
    print(f"  Final value             ${b['final_value']/1e6:<17.2f}M ${r['final_value']/1e6:<17.2f}M")

    sharpe_imp = ((r['sharpe'] - b['sharpe']) / b['sharpe']) * 100
    wr_imp = ((r['win_rate'] - b['win_rate']) / b['win_rate']) * 100
    print(f"\n  Improvements:")
