...
"""
================================================================================
GUNGNIR v32.0.0 — ALLFATHER NEXT-GEN PHASE READOUT PREDICTION ENGINE
================================================================================
...
import csv, json, math, os, re, sys, hashlib, warnings
...
def engineer_v31_features(row, ctgov_lookup=None, journey_index=None):
    ...
    if ctgov:
        ...
        # Hash-based estimates for missing CT.gov data
        ...
        features["ctgov_enrollment"] = math.log(100 + h * 400)
        features["ctgov_n_arms"] = 2 if phase >= 2 else 1
        features["ctgov_is_randomized"] = 1 if phase >= 2 and h > 0.3 else 0
        features["ctgov_is_double_blind"] = 1 if phase >= 3 and h > 0.4 else 0
        features["ctgov_is_placebo"] = features["ctgov_is_double_blind"]
        features["ctgov_masking_rigor"] = 2 if features["ctgov_is_double_blind"] else 0
        features["ctgov_has_dmc"] = 1 if phase >= 3 and h > 0.5 else 0
        features["ctgov_ep_hard"] = 1 if phase >= 3 and h > 0.6 else 0
        features["ctgov_ep_surrogate"] = 1 - features["ctgov_ep_hard"]
        features["ctgov_n_sites"] = int(50 + h * 200)
        features["ctgov_n_countries"] = int(3 + h * 15)
    ...
    # These come from the temporal sponsor index (passed in)
    sponsor_data = row.get("_sponsor", {})
    features["sponsor_n_prior"] = math.log1p(sponsor_data.get("n_prior", 0))
    features["sponsor_success_rate"] = sponsor_data.get("success_rate", 0.5)
    features["sponsor_is_serial"] = 1 if sponsor_data.get("n_prior", 0) >= 5 else 0
    features["sponsor_hot_streak"] = 1 if sponsor_data.get("pos_streak", 0) >= 3 else 0
    features["sponsor_cold_streak"] = 1 if sponsor_data.get("neg_streak", 0) >= 2 else 0
    ...
    features["indication_crowded"] = 1 if indication_count >= 10 else 0

    # --- v32 NEW: EXTERNAL ENRICHMENT (ChEMBL + PubMed) ---
    enrich = row.get("_enrichment", {})
    chembl = enrich.get("chembl", {})

    # ChEMBL molecule properties
    features["is_antibody"] = 1 if chembl.get("molecule_type") == "Antibody" else 0
    features["is_small_molecule"] = 1 if chembl.get("molecule_type") == "Small molecule" else 0
    features["is_adc"] = 1 if "conjugate" in str(chembl.get("molecule_type", "")).lower() else 0
    features["chembl_first_in_class"] = chembl.get("first_in_class", 0) or 0
    try:
        _mp = float(chembl.get("max_phase") or 0)
    except (ValueError, TypeError):
        _mp = 0
    features["chembl_max_phase"] = min(_mp, 4) / 4.0  # Normalize 0-1
    features["chembl_orphan"] = chembl.get("orphan", 0) or 0
    features["chembl_has_data"] = 1 if chembl.get("chembl_id") else 0
    features["n_mechanisms"] = min(enrich.get("n_mechanisms", 0), 5)

    # PubMed publication count (evidence base proxy)
    pub_broad = enrich.get("pubmed_count_broad", 0)
    pub_specific = enrich.get("pubmed_count_specific", 0)
    features["log_pubmed_broad"] = math.log1p(max(pub_broad, 0))
    features["log_pubmed_specific"] = math.log1p(max(pub_specific, 0))
    features["pubmed_high_evidence"] = 1 if pub_broad >= 50 else 0
    features["pubmed_low_evidence"] = 1 if pub_broad <= 2 else 0

    # --- v32 NEW: SPONSOR × OTHER INTERACTIONS ---
    features["sponsor_sr_x_phase3"] = features["sponsor_success_rate"] * features["is_phase3"]
    features["serial_x_small"] = features["sponsor_is_serial"] * (features["is_micro"] + features["is_small"])
    features["fic_x_small"] = features["chembl_first_in_class"] * (features["is_micro"] + features["is_small"])
    features["antibody_x_onc"] = features["is_antibody"] * features["ta_oncology"]
    features["evidence_x_phase3"] = features["log_pubmed_broad"] * features["is_phase3"]
...
def main():
    ...
    print("=" * 80)
    print("GUNGNIR v32.0.0 — ALLFATHER NEXT-GEN TRAINING PIPELINE")
    print("=" * 80)
    ...
    for re_ev in readout_events:
        ...
        merged.append({
            ...
            "nct_id": "",  # Will try to match from cache
        })
    ...
    # Step 3c: Load external enrichment (ChEMBL + PubMed) (v32 NEW)
    v32_enrichment = {}
    if os.path.exists(V32_ENRICHMENT):
        with open(V32_ENRICHMENT) as f:
            v32_enrichment = json.load(f)
        print(f"  v32 enrichment: {len(v32_enrichment)} drugs")
    else:
        print(f"  v32 enrichment: NOT FOUND (will use defaults)")

    # Attach enrichment data to events by normalized drug name
    # Enricher uses normalize_drug() -> .lower()[:40], so replicate that here
    def _norm_drug_key(name):
        """Match the enricher's normalize_drug().lower()[:40] logic."""
        if not name: return ""
    ...
    for ev in sorted_merged:
        drug = ev.get("drug", "")
        drug_key = _norm_drug_key(drug)
        # Try exact match first, then first word
        enrich = v32_enrichment.get(drug_key, {})
        if not enrich and " " in drug_key:
            first_word = drug_key.split()[0]
            enrich = v32_enrichment.get(first_word, {})
        ev["_enrichment"] = enrich

    enriched_count = sum(1 for ev in sorted_merged if ev.get("_enrichment", {}).get("chembl", {}).get("chembl_id"))
    pubmed_count = sum(1 for ev in sorted_merged if ev.get("_enrichment", {}).get("pubmed_count_broad", 0) > 0)
    print(f"  ChEMBL matched: {enriched_count} events | PubMed matched: {pubmed_count} events")
    ...
    deploy = {
        "version": "32.0.0",
        "codename": "Allfather_NextGen",
        "architecture": "4-model meta-ensemble (Ridge_Binary + ElasticNet + Ridge_GOOD + Ridge_CRASH)",
    ...
    print(f"\n{'='*80}")
    print(f"GUNGNIR v32.0.0 TRAINING COMPLETE")
    print(f"{'='*80}")
