...
"""
================================================================================
GUNGNIR v36.3.0 KAIZEN — Regularization + Temperature Tuning
================================================================================
...
DEPLOY_JSON = os.path.join(DATA_DIR, "gungnir_v36_3_deploy.json")
...
def main():
    ...
    print("=" * 80)
    print("GUNGNIR v36.3.0 KAIZEN TRAINING PIPELINE")
    print("=" * 80)
    if not HAS_XGB:
    ...
    for split_name, test_start, test_end in splits:
        ...
        # Model 1: L2 Ridge for P(positive) — v36.1 KAIZEN: C=0.5 (more regularization)
        m1 = LogisticRegression(C=0.2, penalty="l2", solver="lbfgs", max_iter=2000)
        m1.fit(X_tr, y_train)
        p1 = m1.predict_proba(X_te)[:, 1]

        # Model 2: L2 Ridge for P(GOOD+)
        m2 = LogisticRegression(C=0.2, penalty="l2", solver="lbfgs", max_iter=2000)
        m2.fit(X_tr, y_gp_train)
        p2 = m2.predict_proba(X_te)[:, 1]

        # Model 3: L2 Ridge for P(CRASH)
        m3 = LogisticRegression(C=0.2, penalty="l2", solver="lbfgs", max_iter=2000)
        ...
        # Model 5: XGBoost for P(positive) — v36.1 KAIZEN: more regularized XGBoost
        ...
        # v36.1: 55/15/30 Ridge+ElasticNet+XGBoost blend (more Ridge weight)
        p_meta = 0.55 * p1 + 0.15 * p4 + 0.30 * p5
        ...
        T = 0.90  # v36.1 KAIZEN: slightly less aggressive temperature scaling
    ...
    m1_final = LogisticRegression(C=0.2, penalty="l2", solver="lbfgs", max_iter=2000)
    m1_final.fit(X_full, y_bin)

    # Model 2: P(GOOD+)
    m2_final = LogisticRegression(C=0.2, penalty="l2", solver="lbfgs", max_iter=2000)
    m2_final.fit(X_full, y_gp)

    # Model 3: P(CRASH)
    m3_final = LogisticRegression(C=0.2, penalty="l2", solver="lbfgs", max_iter=2000)
    ...
    XGB_PATH = os.path.join(DATA_DIR, "gungnir_v36_3_xgb.json")
    ...
    deploy = {
        ...
        "meta_weights": {"ridge_binary": 0.55, "elasticnet": 0.15, "xgboost": 0.30},
        "xgb_model_path": "gungnir_v36_3_xgb.json",
    ...
    print(f"\n{'='*80}")
    print(f"GUNGNIR v36.3.0 KAIZEN TRAINING COMPLETE")
    print(f"{'='*80}")
