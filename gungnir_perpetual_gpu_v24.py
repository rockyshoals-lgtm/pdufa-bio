...
"""
╔══════════════════════════════════════════════════════════════════════════╗
║  GUNGNIR v24 — PERPETUAL GPU PHASE READOUT OPTIMIZER                   ║
...
║                                                                        ║
║  PREVIOUS BEST (v23 gen 586):                                          ║
║    AUC: 0.895 | Brier: 0.1296 | Acc: 0.818 | Fitness: 0.898          ║
╚══════════════════════════════════════════════════════════════════════════╝
"""
...
try:
    ...
    print(f"[GUNGNIR v24] GPU: {GPU_NAME} | VRAM: {GPU_VRAM_TOTAL_GB:.1f} GB | ENABLED")
except (ImportError, Exception) as e:
    import numpy as cp
    print(f"[GUNGNIR v24] GPU: DISABLED ({e}) — running on CPU")
...
OUTPUT_BEST = "gungnir_v24_best.json"
OUTPUT_HISTORY = "gungnir_v24_run_history.jsonl"
OUTPUT_HALL_OF_FAME = "gungnir_v24_hall_of_fame.json"
OUTPUT_PROMOTION_LOG = "gungnir_v24_promotions.csv"
SEED_CONFIG = "best_config_v23.json"
FEATURE_MATRIX_CACHE = "gungnir_v24_feature_matrix.npz"
...
V23_BEST_FEATURES = {
    ...
    "safety_clean": -1.1365853548049927,
    "ta_CNS": 0.017890293151140213,
...
def load_seed():
    if os.path.exists(OUTPUT_BEST):
        try:
            d=json.load(open(OUTPUT_BEST)); v=config_to_vec(d); print(f"[SEED] Resuming v24: gen={d.get('generation',0)}, fit={d.get('fitness',0):.6f}"); return v, d.get("generation",0)
        except: pass
    if os.path.exists(SEED_CONFIG):
        try:
            d=json.load(open(SEED_CONFIG)); v=config_to_vec(d); print(f"[SEED] v23 seed: fit={d.get('fitness',0):.6f}"); return v, 0
        except: pass
    print("[SEED] Using hardcoded v23 canonical"); return CANONICAL_VEC.copy(), 0
...
def run():
    print(f"\n{'='*72}\n  GUNGNIR v24 — PHASE READOUT OPTIMIZER\n  Features: {N_PARAMS} | GPU: {GPU_AVAILABLE}\n{'='*72}\n")
...
if __name__=="__main__":
    print(f"[GUNGNIR v24] Python {sys.version.split()[0]} | NumPy {np.__version__}"+(f" | CuPy {cp.__version__}" if GPU_AVAILABLE else "")+f" | Params: {N_PARAMS}")
    run()
