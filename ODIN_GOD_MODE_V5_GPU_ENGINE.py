#!/usr/bin/env python3
"""
ODIN GOD MODE V5 — GPU CONFIG SEARCH ENGINE (RTX 4070 / CUDA)
------------------------------------------------------------
Goal:
  Ultra-fast Monte Carlo / random-search over ODIN-style scoring parameters,
  evaluated on labeled FDA PDUFA outcomes (1349 events) with a fused CUDA kernel.

Inputs:
  - Prefer: ODIN_PDUFA_1349_GPU_READY.npz  (fastest load; contains precomputed feature arrays)
  - Or:     ODIN_PDUFA_1349_GPU_READY.csv  (human-readable)

Outputs:
  - JSON of top configurations (by objective + constraints)

Notes:
  - This script is designed for CuPy + CUDA. Install:
      pip install cupy-cuda12x
  - Safe default: optimize on TRAIN split, then score winners on VAL/TEST separately.
...
# --------------------------
# Kernel design:
# - 1 block per configuration (config_idx = blockIdx.x)
# - 256 threads reduce over N samples to compute Brier + confusion matrix.
# - Avoids materializing (N x B) prediction matrices in VRAM.
...
void fused_score_kernel(
    ...
    const float* __restrict__ adcom_pct,   // already normalized 0..1 and zeroed if no AdCom
    ...
    for (int i = tid; i < N; i += blockDim.x) {
        ...
        if (class1_cmc[i] == 1) {
            ...
            // Stack (simple linear; tune w_stack range)
...
class ODINRTXV5Engine:
    ...
    def _default_bounds(self) -> Dict[str, Bounds]:
        # Ranges intentionally wide; narrow after Phase 1.
        return {
            "p_base": Bounds(0.70, 0.92),
            "p_threshold": Bounds(0.50, 0.90),
            ...
            "w_mfg_pen": Bounds(-0.70, -0.20),
            "w_mfg_amp": Bounds(1.0, 2.0),
            "i_mfg_inexp": Bounds(0.9, 1.5),
            ...
            # P003 trap penalty (should stay negative)
    ...
    def _load_data(self) -> None:
        ...
        if path.lower().endswith(".npz"):
            ...
            if "outcome_binary" in df.columns:
                ...
                raise ValueError("CSV must contain outcome_binary or outcome")
        ...
        crls = self.N - approvals
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ Target distribution: {approvals} approvals, {crls} non-approvals ({approvals/self.N*100:.1f}% approval rate)")
    ...
    def _fill_uniform(self, buf: cp.ndarray, low: float, high: float) -> None:
        """Fill existing buffer with uniform(low, high) defensively across CuPy versions."""
        try:
    ...
    def _objective(self):
        ...
        specificity = self.out_tn / (self.out_tn + self.out_fp + 1e-9)

        valid = (precision >= self.precision_min)
        if not cp.any(valid):
            return None
        ...
        s = specificity[idx]
        fpc = self.out_fp[idx].astype(cp.float32)

        # Composite objective
        obj = ((1.0 - b) * 0.50) + (s * 0.30) + ((1.0 - cp.minimum(fpc / 25.0, 1.0)) * 0.20)
        return idx, obj
    ...
    def run(self, iterations: int, report_every_s: float = 2.0) -> None:
        ...
        print(f"\n🔥 SEARCH: {total:,} iterations ({batches:,} batches) | batch={B:,} | split={self.split} | precision_min={self.precision_min}")
        ...
        for bi in range(batches):
            ...
            if now - last >= report_every_s:
                ...
                best = self.top[0]["metrics"]["brier"] if self.top else None
                best_str = f"{best:.5f}" if best is not None else "—"
                print(f"\r   [{pct:5.1f}%] {processed/1e9:.2f}B | {rate_m:6.1f} M it/s | best_brier={best_str}", end="", flush=True)
                last = now
    ...
    def save(self, out_json: str) -> None:
        ...
        for i, r in enumerate(self.top, 1):
            m = r["metrics"]
            print(
                f"#{i:02d} score={r['score']:.5f} | brier={m['brier']:.5f} | prec={m['precision']:.3f} | "
                f"spec={m['specificity']:.3f} | FP={m['fp']} FN={m['fn']} | thr={r['params']['p_threshold']:.3f}"
            )
        print("-" * 90)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data", type=str, default="ODIN_PDUFA_1349_GPU_READY.npz",
                   help="Path to .npz (recommended) or .csv dataset.")
    ...
    p.add_argument("--precision_min", type=float, default=0.94, help="Constraint: minimum precision.")
    p.add_argument("--topk", type=int, default=10, help="Keep top K configs.")
    p.add_argument("--out", type=str, default="ODIN_TOP_CONFIGS_V5.json", help="Output JSON path.")
...
def main() -> None:
    ...
    if not engine.top:
        print("❌ No configs met constraints. Try lowering --precision_min or widening bounds.")
        sys.exit(2)
