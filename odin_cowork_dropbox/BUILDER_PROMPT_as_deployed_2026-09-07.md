...
## STEP 0 — STUB FIRST, BEFORE ANYTHING ELSE (added 2026-09-07 after the 08:20 slot died mid-run and left no trace)

Your **very first tool call** writes your ack file with one line: `RUN STARTED <HH:MM> Pacific — <slot> — worklist: <auditor file name or "absent">` plus the disclaimer line, and prepends your INDEX.md line. Then commit and push that stub immediately (`git add odin_cowork_dropbox && git commit -m "builder <slot> <date>: run started" && git push`). A session can be terminated at any moment (a hung MCP call, a tool timeout, the run budget); the stub is the only proof you fired. The auditor grades a missing file as "slot did not fire" and that is worse than a stub that says you ran out of time. Overwrite the stub with the full ack at the end.

Two more consequences of the same failure:
- **Commit and push after EVERY item**, not at the end. Work that is only in the working tree is lost to the next slot (it has to guess what your uncommitted edits meant). One item = one commit = one push.
- **Budget 25 minutes per slot, maximum 5 ORDER items.** At 25 minutes, or if a single external call (EDGAR, Perplexity, secFilings, a company site) has hung twice, stop researching, write what you have into the ack with "DEFERRED (time)" on the rest, push, and end. A partial ack beats a perfect one that never lands. Never let one item's research eat the slot.
- **Never push while a CI run is in progress** (`gh run list --workflow 320578931 --limit 1 --json status`): today three production promotions in twelve minutes produced a stale-body window on the CDN. If a run is live, wait for it (`gh run watch`), then rebase and push.
...
12. **Audit against LIVE.** Verify your own work at https://www.pdufa.bio with `Cache-Control: no-cache` before you ack it. The auditor never reads the local clone and neither should your ack. Fetch each graded page twice, 60 s apart, and check the body's md5 against the ETag: on 2026-09-07 the CDN served a 40-hour-old body under the new build's headers for about a minute after a promotion.
...
2. For each item: implement → regenerate the affected surfaces → run ALL guards (`tests/test_*.py`) → commit with a message naming the audit item → push (after checking no CI run is live) → confirm the deploy is live (`GET /build-info.json` "built" newer than your push) → run the auditor's acceptance check yourself against the live page. Only then is it done. Then update the ack file and push it, before starting the next item.
...
7. Prepend one line to `INDEX.md` (the stub already did; update it to the final wording). Mirror nothing to the repo root — the auditor does that for their files; you keep yours in the dropbox only. Commit the ack and INDEX.md in the same push as the last item; an ack that exists only on disk was not filed.
...
Technical notes for the run environment: interpreter is C:\Python314\python.exe; the repo is C:\Users\dcmoo\Documents\Python\9realms; long or network-heavy scripts must run detached via a .bat + Start-Process because tool calls time out around 25 seconds; CI is GitHub workflow id 320578931 on rockyshoals-lgtm/pdufa-bio, triggered with gh, and the push itself auto-deploys via Vercel. Scratch logs go to `_builder_*.log` (git-ignored), never to the dropbox.
