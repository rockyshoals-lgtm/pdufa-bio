...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  GUNGNIR PERPETUAL GPU RUNNER v2.1                                         ║
║  Nonstop GPU-accelerated hyperparameter search                             ║
║                                                                            ║
║  v2.1 changes (from 46-cycle / 147K config analysis):                      ║
║    - Fixed is_new_best comparison (was apples-to-oranges)                  ║
║    - L2 grid: explicit negative values (anti-regularization sweet spot)    ║
║    - LR grid: focused on [0.005-0.030] winners zone                       ║
║    - Tier sweep: coverage-aware formula, forces T1 diversity               ║
║    - Fitness: tail calibration penalty (10% weight)                        ║
║    - Strategy: 88% exploitation / 12% exploration                          ║
║                                                                            ║
...
STRATEGY_DISTRIBUTION = {
    # 88% exploitation: start from best + variations
    "platform":       0.40,   # Exact best weights, different lr/l2
    "noisy_small":    0.25,   # Best + small gaussian noise (σ=0.02)
    "noisy_medium":   0.15,   # Best + medium noise (σ=0.08)
    "noisy_large":    0.08,   # Best + large noise (σ=0.20) — bigger jumps
    # 7% semi-exploitation: derived from best
    "half_platform":  0.04,   # 50% of best weights
    "negative":       0.02,   # Negated best (adversarial probe)
    "initial":        0.01,   # Gungnir v4.0 starting weights
    # 5% wild exploration: escape local optima
    "zero":           0.02,   # Tabula rasa
    "random":         0.03,   # Fully random weights
}
...
def gpu_train_batch(X: torch.Tensor, y: torch.Tensor, configs: list,
    ...
    # v2.1: Add tail calibration — penalize models where top-decile predictions
    # don't match actual positive rates (prevents inflated confidence)
    q90 = P_final.quantile(0.90, dim=0).unsqueeze(0)  # 90th percentile per model
    ...
    # Composite: 55% AUC + 20% (1-Brier) + 15% (1-cal_error) + 10% tail calibration
    ...
    for rank, res in enumerate(sorted_r):
        ...
        for t1, t4 in itertools.product(TIER1_BASE, TIER4_BASE):
            ...
            if t1_n < 20 or t4_n < 20:
                ...
                # v2.1: Penalize T1 buckets that are too small (< 15% of data)
                # Forces the sweep to consider T1 boundaries that capture enough events
...
def walkforward_validate(top_configs: list, events: list, device: torch.device,
    ...
    # v2.1: Ensure at least 3 different T1 boundaries reach walkforward
