...
def load_pdufa_data(csv_path: str, weights_json: dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for row in rows:
        ...
        if outcome not in ('approved', 'crl', 'positive', 'negative'):
            continue
        y_val = 1.0 if outcome in ('approved', 'positive') else 0.0
...
class PerpetualHoningEngine:
    ...
    def run(self, resume=True):
        ...
        print(f"   Train: {len(X_train)} ({y_train.mean():.1%} pos, {d_train.min()}→{d_train.max()})")
        print(f"   Val:   {len(X_val)} ({y_val.mean():.1%} pos, {d_val.min()}→{d_val.max()})")
